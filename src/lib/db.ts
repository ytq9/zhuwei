 ...
 * - **Neon**: apply pending `migrations/*.sql` on first `getSql()` so a deploy
 *   whose build-time migrate didn't run still has tables before auth hits them.
 *
...
// Kick bootstrap as soon as this module loads in Node, including Neon, so the
// first auth/page request does not race an empty database.
