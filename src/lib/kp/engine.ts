...
async function lockPlace(
  ...
  try {
    await sql.query(
      `update game_states
       set npc_flags = jsonb_set(coalesce(npc_flags, '{}'::jsonb), '{kpBusyPlaces}', $2::jsonb, true),
           kp_busy = true,
           updated_at = now()
       where room_id = $1`,
      [roomId, JSON.stringify(busy)],
    );
  } catch {
    await sql`
      update game_states
      set kp_busy = true, updated_at = now()
    ...
    await sql.query(
      `update game_states
       set npc_flags = jsonb_set(coalesce(npc_flags, '{}'::jsonb), '{kpBusyPlaces}', $2::jsonb, true),
           ...
           updated_at = now()
       where room_id = $1`,
      [roomId, JSON.stringify(busy), still],
    );
  } catch {
    await sql`
      update game_states
      set kp_busy = false, updated_at = now()
      where room_id = ${roomId}
    `;
  }
}
