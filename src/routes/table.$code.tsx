...
function TablePage() {
  ...
  const { seated, isPending } = useSeated();
  if (!seated) {
    return (
      <main className="flex min-h-dvh flex-col bg-bg">
        ...
        <div className="m-auto w-full max-w-md px-6 pb-24">
          {isPending ? (
            <p className="text-center text-sm text-muted">正在确认座位……</p>
          ) : (
            <AuthPanel mode="in" embed next={`/table/${code.toUpperCase()}`} />
          )}
        </div>
      </main>
    );
  }
  return <TableShell code={code.toUpperCase()} />;
}

function TableShell({ code }: { code: string }) {
  const q = useQuery({
    queryKey: ["table", code],
  ...
  return (
    ...
    >
      <header className="flex shrink-0 items-center justify-between px-4 py-3 md:px-6">
        <Link to="/hall" className="font-display text-lg">
          烛帷
        </Link>
        <AccountChip />
      </header>
      ...
      >
        ...
        {data && !data.ok && !("left" in data && data.left) && (
          <div className="m-auto grid gap-3 text-center">
            ...
            <Link to="/hall" className="text-sm text-brass hover:text-fg">
              回酒馆
            </Link>
          </div>
        )}
