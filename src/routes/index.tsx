    ...
    <main className="min-h-dvh bg-bg text-fg">
        ...
        <div className="overflow-hidden rounded-[28px] border border-border">
          <div className="relative aspect-[16/11] min-h-[320px] md:aspect-[16/8]">
            ...
            <div className="absolute inset-x-0 bottom-0 p-6 md:p-10">
              <p className="mb-2 text-xs tracking-[0.28em] text-brass uppercase">
                D&D 5E · 3 级 · AI KP
              </p>
              <h1 className="max-w-xl font-display text-4xl leading-tight md:text-6xl">
                帷幕后，
                <br />
                烛火未灭
              </h1>
              <p className="mt-4 max-w-lg text-sm leading-relaxed text-muted md:text-base">
                朋友围坐，你开口，骰子落地。AI
                当 KP：叙事、裁决、对抗分席。失败是真的失败，秘密不会从旁白里漏出来。
              </p>
              <div className="mt-6 flex flex-wrap gap-3">
                <Button asChild size="lg">
                  <Link to="/hall">进入酒馆</Link>
                </Button>
                <Button asChild variant="ghost" size="lg">
                  <Link to="/login">登录</Link>
                </Button>
                <Button asChild variant="ghost" size="lg">
                  <a href="#module">今晚的案子</a>
                </Button>
              </div>
            </div>
          </div>
      ...
      <section className="mx-auto grid max-w-6xl gap-4 px-5 pb-16 md:grid-cols-3 md:px-10">
        {[
          {
            t: "完整 5e 建卡",
            d: "种族、职业、子职、点买、背景、技能、法术。3 级开团，建卡本身就是第一场戏。",
          },
          {
            t: "语音进出",
            d: "按住说话，转写成字，你确认后再进桌。KP 用有感情的嗓音读旁白，不剧透。",
          },
          {
            t: "房间与存档",
            d: "房主开房、房间码拉朋友。章节制，短团长团都能续。人物卡、线索、日志留在桌上。",
          },
        ].map((c) => (
          <article
            key={c.t}
        ...
        <div className="grid overflow-hidden rounded-[28px] border border-border bg-surface md:grid-cols-2">
          ...
          <div className="p-6 md:p-10">
            ...
            <p className="mt-4 text-sm leading-relaxed text-muted">
              剑湾小镇暮烛镇。老板死在酒窖门口，嘴里塞着一片黑橡叶。三份遗嘱，三个说法。酒窖的门是新钉上的。门缝里有人在哼摇篮曲。
            </p>
            <p className="mt-3 text-sm leading-relaxed text-muted">
              社交、搜查、一场可以谈下来的对峙。线索会断，人会死。KP
              不会为了主线改骰。
            </p>
            <p className="mt-6 text-xs text-subtle">
              2–5 人 · 约 3–5 小时短团 · 可续长线
            </p>
          </div>
        </div>
      </section>
