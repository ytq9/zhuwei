...
写给接手的会话。这些文件不在任何 SPEC 的 `gates` 里，棘轮和 CI 都不跑；2026-09-19 首次运行时 37 个文件 114 个用例红。**2026-09-20 全部修完：45 个 Worker 文件 344 个用例全绿，棘轮 58 个声明的门也全绿。** 下面按文件记录当时的诊断、已落地的产品修复和遗留缺口；再遇到同类问题先读「已裁定的合同」。运行方式：`npm run test:worker -- --file <文件>`（zsh 下多文件用 `${=FILES}` 展开）；整套 45 个文件逐个跑一遍约 20 分钟。

这批修复暴露的共同根因：这些文件停留在回复优先（ADR 0026）与到期尾部（ADR 0030、ADR 0033）之前的世界模型。修复时先问三件事——世界变化是否等回复、骰子属于谁、这份到期义务是谁造成的——再决定是改产品还是改用例。
