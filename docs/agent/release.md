...
仅对 [vNext 生产替换 Goal](vnext-production-todo.md#本次快速开发部署决定2026-09-06)，下文完整冻结命令由针对实际影响的定向检查、审查和必要部署构建替代，暂不执行全项目测试/Lint。120 条统计采用认证、长期 SLO 后置，不记为通过；8k/16k 及百分比压缩指标为优化参考。权限、事实、机械、恢复与分支边界不变。现有 `zhuwei` 及绑定部署已获授权；push、远端 migration、Secrets、新资源仍须各自授权。其他发布仍遵循下文原流程。
