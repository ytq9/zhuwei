...
这一批把第 1、2、3 层连成了一条真实链：承诺 → 计划 → 到期执行 → 痕迹入正史 → 玩家看见。第三句（到账台前查看）没发出去——批次脚本把每 HTTP 上限硬编码成 5，而计划里写的是 7；等待这一句正好用了 7 次调用，脚本按自己的规则停批。回执见 [round88](../receipts/vnext-round88-validation.md)。
