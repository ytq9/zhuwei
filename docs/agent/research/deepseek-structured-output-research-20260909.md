...
历史 [round87](../receipts/vnext-round87-validation.md) / [round89](../receipts/vnext-round89-validation.md) 还记录过缺少 required 字段的 `{}` 返回；[round84](../receipts/vnext-round84-validation.md) / [round86](../receipts/vnext-round86-validation.md) 记录了分支额外字段或引用 enum 违规。这些是旧批次记录，本次未重新调取其私有原始网络请求，也未新增真实调用；不能将本次路由缺陷直接定为每个旧失败的原因，也不能把所有失败统一归因于供应商。
