...
`services.py shutdown` 两角色 `verifiedAbsent=true`，端口无监听。replay `exactState=true`，stateVersion 16，16 条事件。323 项源码起止 `allEqual=true`。[机器证据](../fixtures/vnext-round91-live-evidence.json)。私有证据在 `/tmp/zhuwei-round91-npc-preparation/evidence`。
