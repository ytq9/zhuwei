...
结构 canonical hash 忽略对象成员顺序，所以这次对照的 canonical requestHash 相同，实际 wire SHA256 不同；证据单独记录二者。最终 workflow 在真实采样后补充呈现 hash 和 Room 精确顺序校验，最终 Provider 请求逐字等于成功对照；最终元数据没有另做真实调用。详情见 [脱敏证据](../fixtures/vnext-round9-live-evidence.json)。原始请求、响应、Cookie 只留于本机私有 `/tmp/zhuwei-vnext-round9-*` 文件，未入库。
