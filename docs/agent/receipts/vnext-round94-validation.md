...
`services.py shutdown` 两角色 `verifiedAbsent=true`。replay `exactState=true`，0 事件。389 项源码起止 `allEqual=true`。[机器证据](../fixtures/vnext-round94-live-evidence.json)。私有证据在 `/tmp/zhuwei-round94-npc-preparation/evidence`。
