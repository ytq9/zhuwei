...
- 用户明确批准 [schema 补取合同](../proposals/schema-retrieval-contract-proposal.md)。SPEC 0015 §6.1、SPEC 0016 §§7.2/10/12 与规格索引已同步。
...
脱敏详情见 [live evidence](../fixtures/vnext-round8-live-evidence.json)。原始请求、回复和会话仅留在本机 `/tmp/zhuwei-vnext-round8-*` 的私有测试文件中，未入库。
