...
[脱敏证据与产物SHA](../fixtures/vnext-round41-live-evidence.json)。命令 `node --import tsx /tmp/zhuwei-diagnostics-room-live-41b.mts --execute`，exit1；日志 `/tmp/zhuwei-diagnostics-room-live-41b.log`；原始请求、响应与usage在 `/tmp/zhuwei-diagnostics-room-live-41b-private/`，未进入公开DTO。未部署、push或远端修改。
