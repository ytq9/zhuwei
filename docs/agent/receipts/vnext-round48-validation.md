...
命令 `node --import tsx /tmp/zhuwei-review-candidate-48.mts --execute`，exit1。日志 `/tmp/zhuwei-review-candidate-48.log`，私有原始请求/响应 `/tmp/zhuwei-review-candidate-48-private/`。三个直接产品源及两个候选实现文件在批次前后SHA一致。[原始结果、事实格审计、SHA及费用](../fixtures/vnext-round48-live-evidence.json)。
