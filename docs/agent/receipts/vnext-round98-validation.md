...
`services.py shutdown` 两角色 `verifiedAbsent=true`。replay `exactState=true`，stateVersion 11，11 事件。389 项源码起止 `allEqual=true`。[机器证据](../fixtures/vnext-round98-live-evidence.json)。私有证据在 `/tmp/zhuwei-round98-npc-preparation/evidence`。
