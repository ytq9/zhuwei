...
2026-09-07，基线 `81c3b1e`。承接 交接 §7 第 1 条（临时交接文件，已删除）：要让 KP 在等待里把「敲账台两下」这类只存在于上下文的承诺还给玩家，等待必须先有旁白。round61 的 [被动时间回执](vnext-passive-time-validation.md) 出于预算把纯等待做成了只走 Activity 确定性显示、不建模型 audience；这一条现在收窄。
