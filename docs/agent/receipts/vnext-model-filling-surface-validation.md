...
结构化结果见[本批真实证据](../fixtures/vnext-model-filling-surface-live-evidence.json)。原始模型材料只保存在本机 `evidence/precision-private/`，正文核对、费用与重放结果在 `closeout/`。game/capture 两个自有进程及 4330/4331 端口已关闭。
