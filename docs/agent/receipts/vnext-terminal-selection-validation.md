...
修改 schema/provider/guidance 3 个生产文件及 schema-retrieval/provider-room 直接测试。Room 原保存响应重证继续使用同一解析器，无新持久化入口。5 文件全部 SHA 核对、0 冲突，见 [集成记录](../fixtures/vnext-terminal-selection-integration.json)。普通 2 阶段、补取 3 阶段、HTTP 5 调用与一次 correction 不变。
