...
`services.py shutdown` 两角色 `verifiedAbsent=true`。replay `exactState=true`。389 项源码起止 `allEqual=true`。[机器证据](../fixtures/vnext-round93-live-evidence.json)。私有证据在 `/tmp/zhuwei-round93-npc-preparation/evidence`。继续见 round94：本地上限提到 10，跑批脚本单独计数被拦事件。
