...
[脱敏证据](../fixtures/vnext-round65-live-evidence.json)；完整私有原稿、冻结请求、journal、状态与执行日志留在 `/tmp/zhuwei-round65-original-preparation/evidence/`。正常setup、3行动/快照/duplicate/ack、meter、shutdown、extract、replay及source-end均exit0。最初只读摘要探查曾误取fixture嵌套键，随后读取result.state得到精确一致；该探查0API/0写入，不影响正式快照和replay。
