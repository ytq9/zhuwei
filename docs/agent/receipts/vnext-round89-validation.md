...
`services.py shutdown` 两角色 `verifiedAbsent=true`，端口无监听。replay `exactState=true`，stateVersion 14。323 项源码起止 `allEqual=true`。[机器证据](../fixtures/vnext-round89-live-evidence.json)。私有证据在 `/tmp/zhuwei-round89-npc-preparation/evidence`。
