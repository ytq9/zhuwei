...
证据与原始产物 SHA 见 [脱敏证据](../fixtures/vnext-round37-40-live-evidence.json)，费用见 [成本账](../vnext-cost-estimate.md)。私有请求和响应保留在 `/tmp/zhuwei-review-{error-37,compatible-38,thinking-auto-39,json-40}*`，未写入公开 DTO。37 的请求 hash 与 round36 完全相同；37 得到的错误正文仅证明这次服务端拒绝原因，不补造 round36 未保存的正文。

本批调用发生在 social B 集成之前，继承记录的 305 源文件清单仍为 [round34 manifest](../fixtures/vnext-round34-source-manifest.json)。本次仅按原产物记账，未重跑模型、修改旁白产品源码、放宽审核、部署或 push。完整行动及连续游戏稳定性仍未通过。
