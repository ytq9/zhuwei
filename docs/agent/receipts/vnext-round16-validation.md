...
详见[脱敏调用与usage](../fixtures/vnext-round16-live-evidence.json)、[round15精确源码差量](../fixtures/vnext-round15-source-manifest.json)。JSON 中 requestCanonicalObjectSha256 是规范请求对象摘要，不能冒充原 HTTP wire 字节 hash。12次已尝试调用，其中11次有usage：42155输入/15177输出，空闲标价约¥0.1085146；另一次400用量未知，不能记为零。reasoning_tokens 已包含在 completion_tokens 内，不另加计费。
