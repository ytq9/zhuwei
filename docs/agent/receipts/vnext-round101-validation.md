...
`services.py shutdown` 两角色 `verifiedAbsent=true`。extract/replay 记未覆盖（没有房间可读，也不造房补样）。389 项源码起止 `allEqual=true`。[机器证据](../fixtures/vnext-round101-live-evidence.json)。私有证据在 `/tmp/zhuwei-round101-npc-preparation/evidence`。
