...
没有编造抄写过程，两次审核五项 pass。这就是[合同](../proposals/vnext-hours-scale-promise-contract-proposal.md)第 1、2、3 层的真实链。
...
`services.py shutdown` 两角色 `verifiedAbsent=true`，端口无监听。replay `exactState=true`，stateVersion 17，17 条事件。323 项源码起止 `allEqual=true`。[机器证据](../fixtures/vnext-round88-live-evidence.json)。私有证据在 `/tmp/zhuwei-round88-npc-preparation/evidence`。
