...
`services.py shutdown` 两角色 `verifiedAbsent=true`，端口无监听。replay `exactState=true`，stateVersion 0，0 条事件。323 项源码起止 `allEqual=true`。[机器证据](../fixtures/vnext-round86-live-evidence.json)。私有证据在 `/tmp/zhuwei-round86-npc-preparation/evidence`。
