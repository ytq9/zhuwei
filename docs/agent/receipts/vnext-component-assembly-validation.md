...
26 文件差量按基线 SHA 串行集成，0 冲突，保存原文件；[集成清单](../fixtures/vnext-component-assembly-integration.json)包含完整原/新 SHA。直接消费者为 KP schema/类型槽/依赖图/lowering、授权 context/read-set、库存转换/事件/correction、Claims/observer/projector 和 Table 两级公开白名单。parser 为 v26；原 v25 根尾语法修订保持。
