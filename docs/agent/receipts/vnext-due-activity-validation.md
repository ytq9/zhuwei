...
ActorPlan/longSpellcasting 迁入通用尾阶段、stableRecovery自然语言/Room持久化与驱逐、DeathSaveResolved在严格Claims根的启动覆盖、未完成obligation的新协议归档恢复、后台主动旁白runner、更多并发timeline/scene边界均尚未闭合。当前归档拒绝未结due仅防止伪成功，不能视作恢复能力完成。[私有恢复合同提案](../proposals/vnext-recovery-contract-proposal.md)已形成具体方案并请求用户确认与SPEC0011的窄冲突；等待答复期间不修改该规格或实现依赖批准的私人旁白归档。
