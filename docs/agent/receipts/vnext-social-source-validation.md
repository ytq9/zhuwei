...
7 个生产文件是 `proposal-bundle-lowering.ts`、`proposal-context.ts`、`proposal-filling-interface.ts`、`proposal-guidance.ts`、`proposal-provider.ts`、`proposal-schema.ts` 和 Room `vnext-proposal-invocation.ts`；6 个直接测试文件见 [集成清单](../fixtures/vnext-social-source-integration.json)。社会互动、worldFact memory、填写接口、schema 检索、private diagnostics、结构化诊断、表示修订、Provider 和 NPC Room 消费者均纳入定向检查。
