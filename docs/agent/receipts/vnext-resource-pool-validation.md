...
验证：精准red在合法core/combat初态下稳定失败（旧core余额2、预期1）；最终 `npx tsx --test tests/kp-vnext-ability-operation.test.mjs` 19/19 exit0，`npx vitest run tests/kp-vnext-ability-operation-room.test.ts` 5/5 exit0，`npm run typecheck` exit0。共享导出helper新增故做类型检查；未运行全量。精确基线/新SHA、日志与命令见[集成记录](../fixtures/vnext-resource-pool-integration.json)。
