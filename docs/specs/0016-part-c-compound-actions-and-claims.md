...
首轮仅以扁平 `requestedCapabilities` 数组选择类型，不填写 decision 或草稿。小表单标识从同一领域 schema 的填写转换派生；服务器保留实际所选小表单身份，只加载所选执行家族与类型依赖。未知标识、重复 ID 和混合草稿继续拒绝，不将缺失裁决的响应改写为类型查询。`directSuccess`、`check`、`concealedCheck` 是填写表单里 `decision.kind` 的取值而非类型标识：选择或补选中出现时按未选处理，其余所选照常；首轮只选了它们是技术失败；补选剔除后没有新增时按无新增的补选处理（ADR 0064）。
...
Room 在外部调用前保存精确请求及 hash，在下一阶段前保存响应；每次选择、补选与提案填写均绑定同一完整冻结上下文。恢复从已保存响应证明是否已补选，不重读新世界或重新选择版本；部署换版本后尚未冻结的方案按 §9.2 重问。允许补选与只准提交使用互斥的阶段提示词；同一阶段状态决定实际工具集合，恢复同时验证提示词、工具及正文。通用权威与填写说明不另行授予补选权限。

提案物理调用因超时、断连等原因成为结果未知时，普通重复请求只读取既有状态，不自动重新派发。原玩家显式点击恢复后，Room 可为同一预算来源根行动授予至多一次替补调用，适用于选择、补选、填写或修订中的当前未知阶段；不按阶段、submissionId 或子行动重置。资格要求原玩家仍控制该角色、行动尚未冻结方案、确认、随机、执行或提交，原场景作用域和房间分支仍有效，且不处于暂停、删除或归档恢复隔离状态。替补使用原精确请求、模型、上下文、所选类型、草稿及修订证明，复用已完成阶段，不重置补选或修订额度。原未知调用及其未知用量占额保留，替补另记实际调用并占用原来源和房间剩余额度；本例外只增加至多一次物理尝试，不增加语义阶段。

替补准入、原调用失去裁定资格和新派发许可必须原子保存；并发或重复点击只能取得同一结果或等待状态。替补前已落盘的原响应优先复用；替补后晚到的原响应仅作为私有调用及计费证据，不能再参与选择、裁定或提交。调用关联及资格跨 DO 驱逐和归档保持；归档恢复隔离不因本机制解除。替补再次失败、额度不足或状态不再允许时返回明确的不可恢复原因，页面保留玩家输入和故障信息并移除该项无效重试。此例外不扩展到故事创作、NPC 或冻结旁白的未知调用，也不改变任何骰子、资源、事件或已提交结果。

沿用上述已批准调用额度：未补选时普通小表单最多 2 次，实际含已选执行家族的复杂提案最多 3 次且至多一次窄修订；实际发生补选或重复选择时分别最多 3 次和 4 次。第四次仅在保存响应证明补选或重复选择已发生且仍满足原稿重发或窄修订准入时存在；额外选择闲置家族不能为最终纯小表单取得执行家族的修订额度。全部实际调用及 Provider 重试均计入 RootAction 调用、token、费用和延迟预算，原 HTTP 总预算保持；§9.2 的重问轮次单独计算上述阶段次数，实际调用仍计入这些预算。模型采用的统计认证按快速部署决定后置，产品正确性边界不变。补选没有新增任何类型、NPC 视图或记忆（含原样重复首轮选择）时，既不是提案也不是补选：服务器把同一张表单不带选择工具再发一次，占用补选那一次调用，之后只准提交；Room 从保存的响应证明这一轮，不附带修正票（ADR 0065）。未知标识、混合草稿、超出许可的再次补选、不可闭合定义和预算超限是技术失败，不是 clarification、世界内拒绝或成功结果。
...
普通新创作不要求已有内容引用。连续性审核报告与既有事实相悖的具体条目，不生成逐片段逐事实的正证据矩阵；来源主张保留归属，程序严格约束机械与权限。新策略使用服务端绑定精确正文与冻结材料的简短异常报告，协议见 SPEC 0015 §7。初稿接受一次独立审核。纯措辞、声口或文风偏好不单独阻断发布；人物、对象、数量、成本、伤害、保密、来源归属、认知范围或玩家自主权的实质改变仍须阻断，不得包装成表达偏好放行。

审核明确指出可修正的实质问题时，允许同一 Viewer 回复执行一次完整修稿及一次独立复审，通常两次、最多四次物理模型调用。修稿只使用原冻结材料、原候选及合法问题理由；报告不构成新事实或授权。复审只审修稿正文；仍有实质问题、无法判断、报告矛盾或损坏时停止，不无限修稿、追加审核或伪造成功。新结果在世界提交前完成这些调用；结算与全部冻结受众回复一起原子保存。终局失败取消全部未交付候选（包括同次 Activity 的开始与自动时间推进），不删除已固定骰面或调用记账。旧已提交结果的恢复继续只改投递，不重裁、不重掷、不重复资源或时间。

前两阶段已保存的请求和响应保留原身份；新增修稿阶段以独立版本及原回执/Viewer/delivery generation 绑定，恢复复用已完成响应，只有尚未派发且具备原始拒绝证据的阶段才可执行。原调用结果未知时不得重采样。发布正文必须与该流程最终允许发布的精确正文一致。逐受众恢复入口由权威账本判断：旧表达拒绝或尚可完成的有界修稿可继续；已知不可变的拒绝、修稿耗尽或未知物理调用不展示无效重试。原稿、审核理由、修稿与复审保存在既有私有调用证据中；公开响应及普通日志不得携带这些材料。
