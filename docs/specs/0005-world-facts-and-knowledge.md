...
revisions:
  - date: 2026-09-26
    scope: "§6.2：被动察觉只判定在场且能感知该举动的 NPC，玩家角色不参与比较"
  - date: 2026-09-26
    scope: "§6.2：注意档位由 Rules 按权威状态给默认，KP 只为有记录依据的改档写档位；行动者对他人注意力的说法不是依据"
  - date: 2026-09-26
    scope: "§6.2：隐蔽动作由行动者一次检定对比各在场者按注意档位调整后的被动察觉，察觉者得到感知证据"
  - date: 2026-09-25
    scope: "§10：分支图按事件记录核对，不再以哈希判断"
adr: ["0056", "0059", "0062", "0063"]
gates:
...
瞒着在场的人做的动作或说的话，只由行动者做一次检定。可能察觉的人由 Rules 按权威状态确定：与行动者在同一场景、在任、存活、未失去意识且能以本次举动的感官感知它的 NPC（看的举动不含失明者，听的举动不含耳聋者；不含行动者），模型不增删。玩家角色不做被动察觉判定，只从看得见的反应或自己的观察得知。注意档位有四档：看着行动者或有戒心（+5）、在场但未特别注意（0）、分心（−5，例如正在交谈、忙别的、背对、光线暗、离得远）、看不到（不参与比较，例如隔着遮挡、不在视线里）。Rules 按权威状态先给每人默认档位：正在执行活动的人为「分心」，其余为「在场但未特别注意」。KP 只为需要改档的人写档位，并引用本次冻结上下文里关于此人的记录（场景描写与最近事件、此人的状态与身份、光线与遮挡）；行动者对他人注意力的说法不是依据；没有依据的改档无效。叙述中已离开、状态仍在场的 NPC 定为「看不到」并引用离开的依据。档位只用于本次行动，不跨行动保存，也不依赖战术地图坐标。Rules 按各人的属性块计算被动察觉（10 + 感知调整值 + 察觉加值），再加上档位调整；检定结果不低于某人的值，此人没有察觉，否则察觉。察觉者各得一条感知证据，内容是 KP 在裁决里写明的「察觉者会看到什么」；未察觉者不得到任何记录。比较结果不直接告诉行动者，行动者只能从看得见的反应得知谁发现了。
...
- 验收：`tests/kp/campaign/world-campaign.test.mjs`、`tests/kp/campaign/stage4-world-campaign-vertical.room.test.ts`、`tests/kp/knowledge/observer-projection.test.mjs`；20+ 当前多轮纵切待用窄工具/Form/Causal/Room 链重建 （该门已随 [ADR 0034](../adr/0034-remove-the-v5-private-form-proposal-path.md) 退役：现役 vNext manifest 不带 `causal-action-interpreter-2014-v5`）
