...
瞒着在场的人做的动作或说的话，只由行动者做一次检定。可能察觉的人由 Rules 按权威状态确定：与行动者在同一场景、在任、存活且未失去意识的角色（不含行动者），模型不增删。KP 在掷骰前为每人定一个注意档位并写明依据：看着行动者或有戒心（+5）、在场但未特别注意（0）、分心（−5，例如正在交谈、忙别的、背对、光线暗、离得远）、看不到（不参与比较，例如隔着遮挡、不在视线里）。档位只能依据本次冻结上下文里的条目（玩家描述的做法、场景描写与最近事件、光线与遮挡、各人身份）并引用依据；没有依据时按「在场但未特别注意」；叙述中已离开、状态仍在场的 NPC 定为「看不到」并引用离开的依据。档位只用于本次行动，不跨行动保存，也不依赖战术地图坐标。Rules 按各人的属性块计算被动察觉（10 + 感知调整值 + 察觉加值），再加上档位调整；检定结果不低于某人的值，此人没有察觉，否则察觉。察觉者各得一条感知证据，内容是 KP 在裁决里写明的「察觉者会看到什么」；未察觉者不得到任何记录。比较结果不直接告诉行动者，行动者只能从看得见的反应得知谁发现了。
