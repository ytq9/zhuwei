...
- **当前 canonical Profile 源码快照**：Projection Policy `projection-observer-safe-v1` = `sha256:972b82b84594386abc2a988a98afb94e5ec925ee1819bc53cd677c722edf8b91`；基础 Runtime manifest = `sha256:496da17f16d52cbe5dfa3e97facfa8ed7dcf3f4bbb7a882fc0e384d464898051`；canonical genesis golden = `sha256:b34ffa3854aee471b2ba568bcd489974571cd89c191bb5c3b1ad7b82587ea71f`。历史环境 v2 Profile/manifest 分别为 `sha256:702b2559c821a52e1c7d6a137c6b261cec21d6cc513e3c0301b4b5ab007f7c87` / `sha256:0021280335296ecfc5b65a221fec7009550fac96db65925e47daef9f9d4f0456`；新双模式环境 v3 Profile/manifest 分别为 `sha256:1656fd548905d6ea886fd4cf97357a9d67c56422be3a2c6bd281fc93a22b4fe6` / `sha256:4038f09e546eb8a0c925e892634625fe09859d2aeba91f044a8ecae76aa99c57`。最终冻结前仍须由 conformance runner 复核全值。
...
- **SPEC 0015 当前定向证据**：Profile/causal/compound 57/57；workflow/table 25/25；campaign 2/2；concentration 1/1；控制/继任 1/1；动态环境 Room 6/6；静态 corpus + production context 14/14；public action/table 21/21；Viewer recovery 4/4；archive D1 11/11（含 checkpoint prefix、ahead event 与 genesis conflict 防线）、80+ events/48 audits 的 reader→fresh DO 1/1、无当前受控 viewer 的 D1→fresh DO 1/1；Planner 6/6；V3 结构评测 4/4、16/16 hard gates；live harness 13/13；workflow-v2/environment-v4 的 31 交互生产 seam 长轨迹 exit 0、1/1，Viewer recovery + 动态环境两文件 10/10。Wrangler local `0000–0011` 与 checkpoint 写读、SQLite `0010→0011` 写读均成功。120 条报告采用 G2，拒绝无增益的 G3/G4，G5 不适用；这些计数均是当前源码的定向证据，不是冻结发布门。
- **当前仍须单列的未完成证据**：生产源码尚未冻结；最终 `module:check`、`typecheck`、`lint`、Node/Worker 全量门、375/1440 浏览器、远端 D1、部署/流量/三交互冒烟/日志/清理和显式 refspec 推送均未由上述局部测试证明。完整线上 Provider 质量指标由用户后续自行测评，本代理不得把三交互冒烟写成完整评测。
...
| --- | --- | --- | --- |
| TM01 真实 scene geometry | Rules state/events/fold + Geometry Profile + Room archive/replay | `tests/dynamic-environment-room-lowering-v3.test.ts` + production-seam long track | **已实现/定向局部**：自定义 feature materialize、Room 事件、replay/幂等及 archive→fresh DO state/project hash 已覆盖；浏览器仍待 |
...
| 向量 | 唯一责任 | 计划 / 当前真实 runner | 当前状态与证据边界 |
| --- | --- | --- | --- |
| KR01 新房/Interface 边界 | 新 V3 manifest + `room/v3-binding.ts` + Room Action；Rules 仍只有 `step/project/replay` | `tests/v3-room-binding-v3.test.mjs`、environment Profile、Causal interpreter | **已实现/定向证据**：完整 workflow/runtime/module 精确绑定，新旧 manifest/profile 隔离且未知组合 fail closed；冻结 `module:check` 待运行 |
...
| KR07 Body-only/Grounding | `kp/narration-v3.ts` + server-derived metadata | public action/table + Viewer recovery | **已实现/定向证据**：exact `{body}`、公开 readModel/delivery 裁剪、冻结 `renderableClaims` 与显式失败纳入 21/21 + 4/4；浏览器/TTS 旁路待 |
| KR08 action/narration 双状态 | Room Action/API/DTO/UI | public action/table 21/21 | **已实现/定向证据**：五加五公开状态、Proposal 与 Narration 失败映射、已提交行动不回滚/不重复机械已有 seam 证据；真实 UI 五路径待 |
| KR09 逐受众发布/亲历 | Room DO delivery journal + `project(viewer)` | `tests/viewer-narration-recovery-v3.test.ts` + V3 production-seam long track | **已实现/定向证据**：recovery 4/4；31 交互长轨迹 1/1 含 Bob viewer-local retry，证明不重提案/重掷/扣资源；死亡/退役/transfer/revoke 后只允许原 ViewerKey 恢复且不投影当前世界，连带恢复/环境 Room 10/10；浏览器待 |
...
| KR12 KP 自定义动态环境 | `environmental-stunt.v1` + Environment v3 FSM + workflow-v2/environment-v4 manifest + Causal Rules/Geometry/Room/replay | Profile/causal/compound、dynamic Room、production-seam long track | **已实现/定向证据**：动态 Room 6/6；workflow-v2/env-v4 长轨迹 1/1 经生产 Form/Context/validator/compiler 与 Room seam，让任意 KP 自定义 `area-hazard` 对 2 实体完成 trigger/resolve/debris，并让竹骨声屏以 `hazard/areaEffect=null` 完成 `state-only`；旧 env-v3 精确保持旧 modifier；用户已取消具体吊灯专项完成门，浏览器待 |
| KR13 十错误/日志 | API failure DTO + `room/{action,telemetry}.ts` | `structured-telemetry-v3.test.mjs` + public action/table | **已实现/定向证据**：十精确代码、Proposal/Narration 阶段、Planner/RAG 降级和固定白名单已有 runner；生产日志冒烟/全调用点扫描待 |
| KR14 120 gold/量化门 | production-equivalent pure seam + live runner/provisioner | `kp-v3-eval`、`live-kp-eval-*` | **部分满足**：结构报告关键 240/240、required 360/360、首次简单 86/88、compound 31/32、最终/route 120/120、故障 5/5；估算 token p50/p95=4918/5812；本地 D1 RAG fallback 0/120；harness/provisioner 13/13 验证默认 31 interactions 与显式三交互边界。Provider tokenizer、端到端 p95、调用率与真实首次合法在本轮保持未测，由用户自行完整测评 |
| KR15 D1 migration/本地闭环 | `db/schema.ts` → `0008`/`0009` corpus + `0010` scrub + `0011` settled archive checkpoint → local D1/corpus/archive restore | corpus/context + `archive-d1-batches-v2` + `archive-do-resume-v2` | **已实现/定向本地证据**：corpus fresh/upgrade/MATCH/rebuild 14/14；`0011` 由 schema 生成并逐行审查，Wrangler local `0000–0011`、SQLite `0010→0011` checkpoint 写读通过；归档分页/伪造游标/前缀篡改/ahead event/genesis conflict 11/11、D1 reader→fresh DO 1/1、无当前受控 viewer 的 D1→fresh DO 1/1；远端应用待 |
| KR16 浏览器/发布/远端 | 375/1440 QA + 完整门 + 既有 deploy/smoke/cleanup/push guard | 待五路径、部署版本/日志/清理、三交互线上冒烟、远端 cloudflare 与 main SHA 证明 | **未满足；完整线上测评不在本代理范围** |
...
| P2 权威行动事务 | Goal §三.1、§四；`SPEC 0003`；`SPEC 0015` §§2、6、8、17 | 同一自由行动可澄清、等待玩家选择、等待 KP 修订、可靠重试；骰前参数冻结，刷新/重试不重复骰、扣资源或提交；冲突只影响相关作用域；错误可公开更正 | `RootAction`、`PreparedAction`、`PendingInput`、冻结提案、`RandomnessRequested` / `DiceRolled`、连续事件、scope heads、幂等 `PublicReceipt`、活动分支与更正记录 | 旧 Profile 保留六种 Outcome；新 Profile 分别投影 action/narration 状态、Receipt、Pending 与专属 Read Model；错误/候选项不泄密 | Room Action；Rules `step/project/replay`；Room Authority `prepare/observe/commit/acknowledge/commitCorrection` | **实际（定向）**：窄修订 5/5、public action/table 21/21、Viewer recovery 4/4，加既有 Room Action/Rules/Room 证据 | **部分满足**：新语义 hash/1+1/无隐式 compound、双状态、冻结受众重试和已提交不回滚已有责任 seam；真实 Provider、冻结全量和浏览器仍待 |
...
| P10 可靠性、纠错、可观测性与评测 | Goal §三.9、§五、§六；`SPEC 0003` §§7–11；`SPEC 0005` §13；`SPEC 0011`；`SPEC 0013` | 同一请求可靠重试且不重复后果；模型/网络/Worker 故障停在稳定点；重启或归档重建后权威状态一致；更正公开且可审计；性能/成本在免费额度预算内；日志不含秘密；多轮 KP 行为达到阈值 | 故障分类、SLO/预算、幂等键与 Receipt、随机承诺及 canonical due-root journal、连续事件/哈希、版本清单、settled archive checkpoint、活动分支/更正、白名单 telemetry、评测记录 | 玩家只见稳定公开失败、重试、当前状态和有权知道的更正；运维只见脱敏关联 ID、分类、耗时桶、版本与哈希 | Room Action；Room Authority；Rules `replay/project`；D1 checkpoint reader/恢复 capability；telemetry；eval runner | **实际（有效/定向）**：G2 故障 5/5、120 结构报告、Viewer recovery 4/4、workflow-v2 长轨迹 1/1、archive D1 11/11、reader→fresh DO 1/1、无当前受控 viewer 的 D1→fresh DO 1/1 | **部分满足，发布仍阻塞**：单调 checkpoint、结清随机、prefix/audit/ahead event/genesis conflict replay、移除成员不复权和本地 migration 已绿；冻结全量、生产日志、浏览器、远端 D1、部署/推送仍须证明。完整线上 Provider 指标由用户自行测评，本代理只做三交互冒烟 |
| P11 权威战术空间与二维地图 | 用户追加 Goal 产品决定/完成标准；`SPEC 0014`；ADR-0012；协作 `SPEC 0003/0005/0010/0012/0013` | 玩家在简单二维图和同源文字读数中看到自身、可见单位、已知障碍/门/地形/持续区/占位/高度/掩护，提交有序路径或区域原点/方向；隐藏实体/障碍不从 preview/DOM/错误泄漏；地图不可用仍可操作 | scene geometry、实体 position/footprint/elevation/height、EnvironmentDefinition/State、portal/destructible/terrain/zone、实际通过路径、Ability/Effect、内部全目标集合、版本/Profile/branch 与连续 WorldEvent | `TacticalProjection` 由 `project(viewer)` 生成并同时供地图、文字、ARIA、preview；只含 Viewer 已知子集，GM geometry/实际隐藏 targets 留在 Rules/Internal | 地图/自然语言 → Room Action → Rules `step` → Room DO 原子提交/权威随机 → `project/replay`；preview 作为同一 projector query；页面只作 Adapter | **实际（局部）**：G01–G15；新增环境 Profile/Builder 14/14、Causal/环境 Rules 8/8、动态环境 Room 6/6；归档 D1 11/11、reader→fresh DO 1/1、无当前受控 viewer 的 D1→fresh DO 1/1 | **局部已实现，浏览器/最终门待**：Environment FSM、破坏/区域、隐藏 target 裁剪、Room/replay 与灾备恢复已有定向证据；完整 Tactical Projection/preview 成对页面、路径/区域输入、双视口及最终门仍待 |
| P12 私有 Proposal、Context/RAG、逐受众叙述与动态环境 | 用户本 Goal；`SPEC 0015`；ADR-0014；DEC-036–044 | 玩家仍只说自然语言；复杂行动不因小表受限；已提交行动在 KP 回复失败时保留；逐观察者独立 body；任意环境想法由 KP 自定义且不按对象名/archetype 派发 | Form/Context/Causal program、RootAction/Receipt、双状态、Audience、`state-only|area-hazard`、exact workflow-v2/env-v4/character-proficiency；D1 FTS/checkpoint 仅派生/灾备 | Required/NPC Viewer 经 `project`；Narration 只用冻结 claims；隐藏 targets 只留内部；旧 env-v3 不被重解释 | Room Action → Form/Context/retrieval/compiler → Rules `step` → Room DO → `project` → body publish；D1 只静态索引/settled archive | **实际（定向）**：KR01–KR15；G2 结构门；workflow-v2/env-v4 31 交互长轨迹 1/1；Viewer+环境 10/10；archive D1/restore 11/11+1/1、无当前受控 viewer 1/1 | **部分满足，发布仍阻塞**：G3/G4 拒绝、G5 不适用；本地 migration 完成，冻结全量、浏览器、远端 D1、部署/三交互冒烟/清理/push 尚待；完整线上测评由用户执行 |
...
| 十二板块与 A–O 无未归属项 | 本矩阵 P1–P12、TM01–TM14、KR01–KR16 和 A–O 每行均有来源、状态、投影、Interface 与测试映射 | **结构映射已列出**：KR01–KR15 与部分 TM 项已有本地实现/定向证据；仍标注待办的 TM/UI/浏览器、KR16 与发布证据继续阻塞完成声明 |
...
| 随机、幂等、故障、恢复、回放、更正 | P2/P10/N 的 DO、归档、分支和模型失败测试 | **部分证据**：最新 randomness/recovery/contest 3 files / 24 tests、Room retry 3/3、archive D1 11/11 与 80+ events/48 audits 的 reader→fresh DO 1/1、无当前受控 viewer 的 D1→fresh DO 1/1 已覆盖并发首写、多波、旧 journal、journal 篡改拒绝、scene settlement lock、单调 checkpoint、D1 前缀重放、ahead event/genesis conflict、完整 audit 集合、移除成员不复权、幂等 Receipt 和归档续传；既有 correction 证据仍须在冻结源码重跑，远端 D1 及线上检查仍待 |
| 20+ 多轮 KP 评测 | 单一连续评测轨迹、验收阈值、逐轮 Receipt/Viewer 证据和评分 | **数量与两代 seam 均满足**：既有单场景 31/31；新增 V3 production seam 长轨迹 1/1 以 15 Intent/Proposal + 15 ACK + 1 Bob retry 完成精确 31 次真实 Room 接口交互，覆盖双环境模式、无重复机械与 replay；仍不能据此宣称真实 Provider/整站完成 |
...
| 战术地图 TM01–TM14 | SPEC 0014 场景 1–14、真实 Room/archive/replay、Viewer indistinguishability、地图/文字/ARIA、375px/1440px、最终冻结门 | **部分满足，仍阻塞**：环境 FSM、破坏/区域目标、自遮挡、隐藏裁剪、Room/replay 与 D1 reader→fresh DO 已有定向证据；完整 Tactical Projection/preview、页面输入、双视口和最终门仍待 |
| Form/Context/RAG/Narration KR01–KR16 | 十 Form、三层 Context、静态重读、1+1、body-only、双状态、逐受众、模型角色/G0–G5、十错误、120 gold、D1/浏览器/发布 | **部分满足，仍阻塞**：KR01–KR15 与 31 交互生产 seam 长轨迹已有定向证据；G2 结构门通过并采用，G3/G4 拒绝、G5 不适用；`0000–0011` 本地 migration/checkpoint 闭环已完成，远端 D1、KR16 浏览器/发布/Git 与部署后三交互冒烟仍待。完整 live Provider 指标由用户自行测评 |
| 本地验证命令 | `module:check`、`typecheck`、`lint`、`test:unit`、`test:worker`、`git diff --check` 的源码 SHA、命令、退出码；部署 build 只由 `cf:deploy` 执行一次 | **未满足**：局部切片命令已有回执；生产源码冻结后的同一 SHA 发布门尚未运行/通过 |
| 迁移、部署、冒烟、日志、推送 | 必要迁移状态与闭环、Cloudflare version/流量/URL/冒烟/日志、`DEPLOY_SOURCE_SHA`、`DELIVERY_SHA`、远端 `cloudflare` SHA、远端 `main` 前后 SHA | **未满足**：`0008`–`0011` 本地 fresh/upgrade/FTS/rebuild/checkpoint、正式 generate 与逐行复核已有证据；远端 migration、部署、流量确认、三交互生产冒烟/日志/清理与显式 refspec 推送尚未完成 |
