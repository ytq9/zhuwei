...
旁人察觉后的当场反应：隐蔽动作的主要察觉对象以外，每个察觉了的 NPC（SPEC 0005 §6.2）在同一次行动中单独调用一次模型，只依据该角色本人的投影（身份、目标、性格、知识和刚看到的内容），决定是否反应以及怎样反应；不反应也是合法结果。反应经 `step` 结算，随本次行动在同一次回复里发布。各察觉者独立决定；一人的反应给别人带来的新感知进入他们的记忆，但不在本次行动里再引发新的反应。某个察觉者的调用失败时，他这次不反应，感知记录保留，行动结果照常发布。每次调用计入本次行动的模型预算。察觉了的玩家角色只得到感知证据，反应由玩家自己决定。
