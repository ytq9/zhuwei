...
revisions:
  - date: 2026-09-26
    scope: "§7：玩家角色不参与被动察觉判定，删去「察觉了的玩家角色只得到感知证据」"
  - date: 2026-09-26
    scope: "§4、§7、§11：NPC 按本人的决定移动场景，提交与玩家相同的 CharacterMoved；交谈中答应马上动身记为承诺，下一次行动开始时离开"
  - date: 2026-09-26
    scope: "§7：察觉隐蔽动作的旁观 NPC 各调用一次模型，按本人投影当场决定是否反应"
adr: ["0059", "0060", "0061", "0063"]
gates:
  ...
  - "tests/kp/npc/npc-movement.room.test.ts"
---
...
- 当前地点（只随本人的移动改变，见 §7）、资源、伤势、装备、关系、承诺和债务；
...
旁人察觉后的当场反应：隐蔽动作的主要察觉对象以外，每个察觉了的 NPC（SPEC 0005 §6.2）在同一次行动中单独调用一次模型，只依据该角色本人的投影（身份、目标、性格、知识和刚看到的内容），决定是否反应以及怎样反应；不反应也是合法结果。反应经 `step` 结算，随本次行动在同一次回复里发布。各察觉者独立决定；一人的反应给别人带来的新感知进入他们的记忆，但不在本次行动里再引发新的反应。某个察觉者的调用失败时，他这次不反应，感知记录保留，行动结果照常发布。每次调用计入本次行动的模型预算。玩家角色不做被动察觉判定（SPEC 0005 §6.2），只从看得见的反应或自己的观察得知。

NPC 场景移动：NPC 的所在场景只随本人的决定改变（履行本人承诺的工作、察觉后的当场反应）。交谈中 NPC 答应马上动身去别处，也记为本人的承诺：这次交谈的结果不写其已离开，玩家下一次行动开始时由本人的工作执行离开。KP 按本人投影写一步移动：去一个已登记场景（模组地点或已创建的动态地点）并写路上耗时，或经一条从当前场景出发、本人看得见的连接走；要去的地方还没有地点时，同一次决定里先创建动态地点和连接，再经这条连接走。Rules 核对该 NPC 是这次的行动者、在任、能移动（未失能、未被擒抱或束缚）、不在遭遇中、目的地可达，提交与玩家移动相同的 `CharacterMoved`：出发即离开原场景，场景、战术地图位置和时间线一起改变。NPC 按本人的时钟出发，到达后用目的地已有角色的时钟（先用玩家的），到达时刻不早于出发加路上耗时，也不早于该时钟的当前时刻。只是离开的决定不另占原地的时间，时长只算出发前在原地做事的时间。原场景的在场者得到其离开，目的地的在场者得到其到达，别处的观察者不得到任何记录。玩家角色仍按 SPEC 0007 §5 移动。
...
- `CharacterMoved`（NPC 本人的场景移动，与玩家移动同一事件，§7）
...
9. NPC 答应去别处办事，下一次行动前按本人的决定离开：原场景不再把他算作在场，地图上不再显示，原场景的人得到他离开，目的地的人得到他到达；去未登记的地方时先有动态地点和连接。
