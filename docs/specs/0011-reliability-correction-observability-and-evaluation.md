...
Room DO 提交后产生待归档标记；Worker/D1 Adapter 把 Room 持有的故事归档（内含完整世界归档：genesis、事件与回执引用）分块写入 D1，再用一次检查点写入指明该房间 epoch 的归档世代、内容、头部事件序号、最后一条事件 id 和活动分支。D1 不保存逐条事件、genesis 或投影审计副本。归档至少保存规则集、事件 schema、模组/定义/Profile 哈希和活动分支；不保存旁白 Delivery Frame。归档按 Room 持有的内容原样复制：导出和写入 D1 时不重放世界，也不核对其中的动作记录、收录回执或投影。

重建流程：

1. 从 D1 检查点取得房间当前的归档世代和内容，读取它的全部分块；
2. 检查世界归档的事件序号从 1 连续、每条事件的父事件是上一条、同一房间和同一组 Profile；
...
- Room DO 从完整日志构造 `CorrectionContext`，以更正时的头部事件序号与最后一条事件 id 为基准，头部变化后的请求拒绝；调用者不能提供状态补丁、事件、MechanicOp 或分支图。
...
- 每个事件的更正审计只记录该事件实际改变的领域记录及其改变前的值：角色与控制、知识、事实、场景、时间线、战斗实体与战斗运行态各集合的条目、定义、Campaign 条目和物品条目；未改变的记录不记录，按需创建的集合记录其此前不存在。每条审计记录另存该事件的载荷，同一根内后续事件按载荷内容核对此前已提交的事件。审计是从事件日志派生的索引。更正按受影响事件逆序恢复这些记录，并同步重建 Room 的待决鉴权索引，不能留下幽灵 encounter、候选或可回答的旧待决。
...
- 更正不得改变房间的 runtime manifest pin。本 Goal 的 authoritative-v2 尚未首次正式发布，冻结发布源码时可以同步更新其待发布 manifest/hash；首次正式发布后，任何会改变回放所得状态或已提交事件解释的实现都必须发行新 manifest/interpreter，禁止在原 pin 下静默替换。是否继续保留旧解释器取决于当时仍在使用的数据合同并须另行裁定，不能预先建立 fallback。
