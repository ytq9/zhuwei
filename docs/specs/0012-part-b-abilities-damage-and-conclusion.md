...
每个独立伤害来源分别触发一次专注体质豁免，DC 为 `max(10, floor(damageTaken / 2))`。`damageTaken` 是免疫、抗性、易伤和其他减免后、分配给临时 HP 或 HP 前的该来源伤害；完全降为 0 时不触发。临时 HP 吸收伤害不取消已经发生的专注豁免。该豁免骰可以与触发它的伤害骰在同一批权威随机中请求；DC 在该来源伤害结算后才确定，伤害完全降为 0 时预取的骰面不被消耗、不产生豁免。
