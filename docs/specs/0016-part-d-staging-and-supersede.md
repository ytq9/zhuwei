...
已发出但结果未知的调用不重新采样；恢复复用已保存的响应、骰子与资源。部署改变版本后的例外见 §9.2。公开 trace 只取已提交结果，ActorPlan 的私有计划字段不进入 Claims。

### 9.2 部署换版本后的未提交工作

部署改变提示、schema 或请求组装后，已保存的模型调用可能出自旧版本。判断依据是：某份工作第一阶段的冻结输入不变，但当前版本的绑定 hash 不同；请求由 Room 自己重建或核对时，重建出的请求不同也算。调用方送来的请求在同一版本下与已保存的不同，仍是冲突，不是换版本。

只有尚未冻结、尚未提交的工作按当前版本重问：

- 玩家行动：方案尚未冻结，即还没有确认、澄清、随机、执行或提交；
- NPC 到期决定：决定尚未冻结或提交；
- 冻结旁白：尚未发布。

已冻结的方案（包括玩家已看到的确认、澄清与检定）和已提交的结果不重问，按冻结内容继续；它们此后不需要模型调用。

重问时，该工作旧版本的全部阶段作为一轮移入私有审计，按当前版本从第一阶段重新开始，每轮使用自己的调用身份。新一轮的阶段与调用次数按 §7.2 和各自合同重新计算，不计入旧版本已用的次数；每次版本变化，每份工作至多重问一轮。旧调用即使结果未知也不再等待，晚到的旧响应只作私有计费证据，不参与裁定。所有实际调用仍计入来源 RootAction 与房间的调用、token 和费用预算。旧版本的阶段与调用跨 DO 驱逐和归档保留。
