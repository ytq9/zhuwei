...
2026-09-07 首轮填写收敛的当前替代：parser v34 首轮只填扁平 requestedCapabilities，第二轮才填所选小表单或执行家族。类型从同一领域 schema/注册表派生，expanded 仅装配所选表单和类型依赖。Room 两轮 user 正文逐字绑定同一完整冻结上下文，保存响应证明阶段；空执行集合不生成无效 direct/check/clarification，未加载种类不能借修订换裁决。普通小表单最多2次，实际含已选执行家族的复杂草稿最多3次且仅一次窄修订，闲置选择不增加预算；HTTP上限保持。该选择可能增加普通动作调用，体积与本地检查不证明真实稳定性。见[当前接口验收](../agent/vnext-flat-selection-validation.md)，[旧混合入口证据](../agent/vnext-selected-schema-validation.md)保留。
...
2026-09-07 已注册能力使用 native abilityOperation 填写面：仅选本人能力、目标/模式或本人施法 Activity，固定费用、骰子和效果来自原注册定义与 Rules；无重新裁决字段。它经现有 atomic native driver 和完整 clarification 冻结继续，遵守已选执行家族最多三次及一次窄修订。正常角色初始化共用原能力注册编译事实源；详见[实现回执](../agent/vnext-native-ability-validation.md)。
