...
- 同日第二批（`npm run gates:check` 首次运行后）：`observer-delivery.room`、`observer-incremental.room`、`room-retry.room` 只保留 vNext 可运行的用例；只剩占位跳过用例的 `environment-zone.room` 删除并从 SPEC 0014 gates 去掉；`world-campaign` 中经 V5 `registerDynamicDefinition` 路径的三个“SPEC 0001 8”用例删除，SPEC 0001 §8 的单测追溯改标在 `tests/kp/world/hazard-product-closure.test.mjs`。`tests/platform/architecture/spec-0001-acceptance.structure.test.mjs` 的登记表改指现存证据，E、J、O 三个场景如实登记为部分覆盖。
- 顺带核实：`authenticatedCampaignAction`（退役、晋升、章节转换）在生产代码里没有调用方，只有 `introduceSuccessor` 有入口；SPEC 0008 的这些生命周期动作在 vNext 房间目前没有生产路径，属于产品缺口，不由删除测试解决。
