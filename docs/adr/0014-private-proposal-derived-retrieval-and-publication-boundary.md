...
> 2026-09-09 vNext 修订：按用户明确决定，未生效 Proposal 不再受首稿语义冻结或服务端等价修复计划限制。KP 接收与唯一填写草稿同路径的精确诊断、同一冻结上下文，以绑定源版本的原子 JSON Patch 或完整替换提交修订稿；已加载类型、玩家意图与既成事实不变。最终方案在确认/随机/执行前冻结，完整重验与一次修订预算不变。当前合同见 SPEC 0016 §7.2；以下保留 V5 历史决策。
