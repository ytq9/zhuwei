...
- 该能力的定义闭包随之删除：`rules/profiles/environment-form-lowering.ts` 把私有 Form 草稿转成 `rules/profiles/environment-definition-builder.ts` 的唯一开放输入，两者共 589 行，被删的 Form 是它们唯一的生产者，六个导出在闭包外零引用。`tests/kp/world/environment-definition-builder.test.mjs` 的四个用例全测这个 builder 本身，没有现役承接者，一并删除。
...
- SPEC 0015 §§2–6 描述的「私有小表 Proposal → 三层 Context Pack → 静态 RAG → 封闭因果程序」管线不再有实现，连同 §10 的 G0–G5 采用门、§13 的量化硬门与 §9 的 `context-planner` 角色自此只解释历史。**退役范围到此为止**：SPEC 0016 §12 明示不整篇替代 SPEC 0015，§7–§9 的 body-only Narration、行动/Narration 双状态、逐受众发布与主 KP 角色边界，以及 §12 的公开错误码与日志白名单仍是现役规则，继续由 SPEC 0015 承载——SPEC 0015 剩下的五道门守的正是这些条款，现役 `narrate` 路径实现它们。（第六道 `selection-amendment` 守的是 §6.1 的补选，那条规则已由 SPEC 0016 §7.2 承接，门同日改挂 0016。）
- 失去的验收证据：`tools/run-kp-v3-eval.mjs` 的 16 项结构硬门（120 条 gold 的引用召回、表单合法性、路由覆盖、故障回退与四项体积门）。这些门量的是 V5 请求的构成，在 vNext 上没有对应对象；vNext 的请求体积没有等价的**离线**门，属于已知缺口。运行时仍有守卫：`kp/vnext/invocation/assemble.ts` 对组装后的请求体执行 26,000 token 预算，超了返回 `PROPOSAL_INPUT_BUDGET_EXCEEDED`。实测数字与三个建门口径见[任务书](../agent/task-vnext-request-size.md)。
