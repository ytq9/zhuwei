...
- **不解决的问题**：`conservative-v1` 计数器未经完整校准，`budget.ts` 自己写明这属于部署资格且尚未进行。棘轮锁住的是「按当前估算口径不再变差」，不是「26,000 这个数字是对的」。
  同日的 [round104](../agent/receipts/vnext-round104-validation.md) 取得第一份真实对照：对逐字节相同的请求，估算 52,915 / 52,889，DeepSeek 实际 49,001 / 49,006，比值 1.080 / 1.079。**方向与设计一致——高估约 8%，不低估。** 两个样本、同一种输入形状，不足以称已校准，也不能外推到旁白或审核；完整校准仍需单独授权。
