...
## 2026-09-11/12 规格体系去重、拆分与归档（能力开发）

- 目标与能力合同：让「当前规则是什么」能在一个地方读到。SPEC 只记现时规则并就地改写，ADR 记一次决定的日期、理由与取代范围且写完不追加，回执记一次验收证据且不再修改。取代必须是编辑而不是追加。
- 散文头部去重：16 份 SPEC 的状态、日期、上位规格、取代范围同时存在于 frontmatter 和散文列表。删 72 行、零新增，每一行都用 diff 核对属于已进 frontmatter 的八类；`产品`、`适用规则`、0011 平台、0013 与 0012 的关系、0015 适用边界、0016 当前边界保留。检测器原先只认 `修订日期：`，补上了 `补充裁定：`、`0.4 修订：`、`schema 选择补充：`、`替代裁定日期：`、`依赖规格：`。
- 索引沉积拆为 ADR：`docs/specs/README.md` 的标题与导航之间压着十段带日期的散文，其中三段还在「## 导航」标题之后。逐条核实其声称的规则**已在 SPEC 正文**（环境叙述在 0001 §§3.3/7/12 与 0016 §8，窄修订在 0016 §7.2，schema 选择在 0015 §6.1 与 0016 §§7.2/10/12，0.4 重置在 0013 §0.4，故事预算在 0015 §§6.1/17）才删，因此是纯去重。新建 ADR 0016–0022，每份记何时、为什么、取代了什么并指向条款。核实中发现索引引用的 `SPEC 0016 §9.2` 从不存在（§9 无小节），ADR 0018 只记 §8.3 并写明。README 24681 → 11105 字节。
- 附录外移与分册：0010/0012/0013/0015/0016 的自主裁定记录、实现映射、交叉审查、完成门、开发期回执共 26297 字移入 `kind: annex` 附录，原位留标题和指针，`§13` 仍可解析；动手前确认被引用条款（0010 O16、0013 §7.1/§7.4/F04、0015 §6.1、0016 §4.2/§4.3）都不在移动范围。随后按条款区间拆成 `kind: part` 分册，编号不变。最大文件 29227 → 9564 字。拆分中 0016 §14 落在所有区间之外被丢掉（条款数 34→33），已补回。
- 工具改动：`spec-trace` 支持多文件规格（part 并入父规格做条款查找、`parts`/`part_of` 双向校验）、字数按单文件且只数正文、superseded 规格跳过体量检查、`stale-gate` 的日期忽略 frontmatter 与 H1 下方元数据列表（两次误报都因此产生）、新增 `gates_verified_on` 让重构后的警告有正当清除方式。新增 `tools/check-doc-links.mjs`（含 `--baseline`）与 `tools/gate.mjs` 的 `docs.brokenLinks` 指标。三个工具的执行包进 `main()` 并登记 `moduleScopeInvocationAllowlist`，`assertNoModuleScopeEffects` 13 → 3。
- 归档：`docs/refactor-log.md` 970 KB 拆成按月文件（`0000-plan` 6 条、`2026-08` 119 条、`2026-09` 214 条），索引降到 1 KB；拆分前后二级标题都是 339 条且逐条同序。`docs/agent` 307 个平铺文件按用途分为 proposals/receipts/fixtures/releases/research，根目录留 11 个指导与当前工作项。
- 规则归位：ADR 0015 正文积了 25 条日期追加。12 条实现记录（纵切、parser v33/v36/v37、承诺生命周期、供应商 HTTP400）移入回执文件，3 条交叉引用到 ADR 0017/0018/0020。核实发现三条产品行为**任何 SPEC 都没写**，经用户同意写成 SPEC 0016 §7.3（单一 decision 填写面）、§8.3 追加（连续性审核只报相悖）、§9.1（到期 ActorPlan 的执行与恢复），ADR 改为记理由并指向条款。
- 验收门：0016 补 4 个门（`kp-vnext-proposal-schema` 23 项、`kp-vnext-narration` 33 项、`kp-vnext-actor-plan-due-claims` 7 项、`kp-vnext-model-call-scope` 6 项），0015 补 `kp-vnext-selection-amendment` 5 项。全部声明前跑过，共 74 项全绿。0010/0012/0013 盖 `gates_verified_on: 2026-09-11`（重构可验证为保文本）；0015/0016 不盖，其 stale 是真问题，改由补门清除。
- 自动化：`.github/workflows/gate.yml` 在 push/PR 上跑硬门加棘轮；`.claude/settings.json` 的 PostToolUse 钩子在编辑 `docs/specs/**` 与 `docs/**/*.md` 后自动跑检查，成功静默、失败退出 2，仓库根由被编辑文件路径推出而非 cwd。新增 `.claude/skills/spec-workflow/` 承载步骤与判断标准，AGENTS.md 与 CLAUDE.md 只留规则与指针。
- 定向检查及退出码：`npm run spec:check` exit 0（0 错误、6 警告，全为 oversize 且超 1%–20%）。`node tools/check-doc-links.mjs --baseline` exit 0（1 条既有断链在基线内）。`node tools/gate.mjs` 八项指标全部持平。钩子逐项管道测试（编辑 spec 触发、编辑源码不触发、不存在的 gate 退出 2 并报 missing-gate-file、造断链退出 1 并点名、从 /tmp 运行仍解析正确仓库），并用哨兵证明真实触发后清除。`stale-gate` 由最高 5 条降到 0。
- 未覆盖：`docs/agent/proposals/vnext-fiction-time-contract-proposal.md` 引用已被 `1357442` 删除的源码文件，是本次之前既有的断链，已登记基线未修。技能触发率未验证——`claude -p` 在本机与用户终端均返回 401，而 `scripts/run_eval.py:88` 丢弃 stderr 且不检查退出码，把调用失败记成「未触发」，产出的 `recall=0%` 是测量失效不是结果，未据此修改描述。深 Module 边界 152 处与单测 98 个失败两项欠账未动，任务书见 `docs/agent/README.md`。
