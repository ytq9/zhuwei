...
- 验证：三个文件 74/74 全过；`node tools/gate.mjs --check --with-gates` 退出 0，54 个声明的门全绿，八项棘轮指标与 22 项 requestSize 全部持平。未推送，因此超时修复能否让 CI 转绿、以及未解释的那一项究竟为何失败，都要等下一次 workflow 运行才能确认，本条不预先声称已修好。
