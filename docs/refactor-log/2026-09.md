...
## gate workflow 首次运行的 14 项 Worker 失败（2026-09-20）

- 症状与范围：推送 `e0706d1` 后，`gate` workflow 首次执行即红，14 个用例失败于 3 个 Worker 文件——`stage3.room` 12 项、`time-passage.room` 1 项、`provisional-reply.room` 1 项。`gh run list` 显示该 workflow 只有这一次记录：它随代码加入仓库后一直没有推送触发，所以这些用例从未在 GitHub runner 上跑过。同一提交本地 45 文件 341 用例全过，事后再次单独复跑这 3 个文件仍 74/74 全过。
- 被排除的假设（各自有证据，不是推测）：依赖差异——`workerd 1.20260820.1`、`@cloudflare/vitest-plugin 1.0.0`、`vitest 4.1.11`、`miniflare 5.20260820.0-alpha`、`wrangler 4.125.0` 五项已装版本与 `package-lock.json` 逐一相同，`npm ci` 不会改变；时区——CI 在 UTC、本地 +0800，以 `TZ=UTC` 复跑 `time-passage` 与 `provisional-reply` 为 38/38 全过；构建产物——CI 在跑门前执行 `npm run build`，但本地 `dist/` 同样存在，`measureGates` 只用它决定是否跳过 HTTP 套件。未能排除的一项是 Node 版本（CI 22.13、本地 24.14）：机器上没有任何 node 版本管理器，docker 守护进程未运行，无法本地复现该轴。
- 测量：以 JSON reporter 取全套 341 个用例的逐项耗时。超过 3s 的 31 项、超过 5s 的 18 项、超过 10s 的 3 项，最慢为 `archive-do-resume` 的「resumes 80+ events through bounded alarms」50.51s。对 CI 失败的 12 项逐个核对其超时预算（正则扫描不可靠，最终以直接阅读 `it(...)` 尾部确认）：9 项继承 vitest 默认 5000ms 而本地耗时 2.13–3.99s（余量 1.25–2.3 倍），`stage3` 行 3852 的两项 `for (const choice of [...])` 显式 20000ms 而本地 15.46s 与 8.58s（余量 1.29 与 2.33 倍）。
- 判断：11/12 的失败可由余量不足解释——GitHub 2 核 runner 对 CPU 密集工作慢 1.5–2.5 倍即越线，而失败名单中 11 项带 eviction 或 after restore，正是驱逐与重放这类序列化密集路径。反证支持同一判断：本地最慢的 50.51s 用例在 CI 通过，它是等闹钟而非占 CPU，不随机器变慢。**不是本次归档造成的回归，是 CI 首次运行暴露的既有状况。**
- 第 12 项另有根因（2026-09-21 查清）：`time-passage` 的「the private due capability rejects a changed root or Activity before any time is spent」本地 0.38s、预算 30000ms，余量 79 倍，超时解释不了。推送超时修复后 CI 由 14 项失败降至这 1 项，并因下条的工具改动首次给出断言：`expected { kind: 'rejected', …(2) } to match object { kind: 'committed' }`，位置为该文件第 492 行的 `resume(stub, root, c)`。根因是用例与 Room 自身闹钟争抢同一条到期工作：`commitDueActivity` 第一道检查即 `work.status !== "pending"` 时返回 `dueActivityUnavailable`，而闹钟先到时会按 `isSupersededTimePassageAdvance` 把该排队完成 `finishDueWork(..., "cancelled")`，**不写任何事件**——所以第 491 行的「事件未变」断言在 CI 上照常通过，紧接着第 492 行的显式 resume 无事可做。本地在第 492 行前插入 6 秒真实延迟即精确复现：`{"kind":"rejected","code":"dueActivityUnavailable","explanation":"The due Activity work is unavailable."}`。**产品行为正确**，闹钟本就该排空到期工作，是该用例假设了自己能抢赢。按仓库既有做法（`multiplayer-publication`、`provisional-reply`、`interrupted-publication` 三个文件已在用）在崩溃之后、否定校验之前加 `state.storage.deleteAlarm()`。前后对照：同样加 6 秒延迟，修复前复现 CI 失败，修复后通过；该用例测的是私有到期能力的拒绝而非谁排空队列，未削弱任何断言。`time-passage` 全文件 17/17，完整门 54/54 全绿。
- 工具缺陷与修复：`tools/gate.mjs` 的 `runWorkerTests` 从 vitest JSON 报告中只取 `a.title`，丢弃 `failureMessages`，因此 CI 只报出哪些用例失败、不报为什么，这正是本次排查受阻的原因。改为在超出基线的门下打印断言与堆栈前 6 行；基线仍只记标题，格式不变。以临时弄红 `room-retry.room.test.ts` 的一处断言端到端验证显示正确，随后还原该文件。
- 修复：`tests/config/worker.config.ts` 补 `testTimeout`/`hookTimeout` 为 60_000——该文件自身注释要求测试环境设置集中于此，这样 `npm run test:worker`、`tools/gate.mjs` 与 CI 共用同一口径；60s 取自套件中已有的最宽松逐用例值。`stage3.room.test.ts` 行 3905 的 `20_000` 提为 `60_000`，因 15.46s 对 20s 是失败名单中最薄的余量。超时是 harness 限制而非行为断言，`fileParallelism: false` 之下真实挂死仍在一分钟内结束。
- 验证与最终结果：本地三个文件 74/74、`node tools/gate.mjs --check --with-gates` 退出 0。推送后 CI 两次运行确认：`4feab58` 由 14 项失败降至 1 项（超时修复解决 13 项，并首次输出该项断言），`d54bc13` 加上闹钟竞态修复后 **run 35524765652 全绿**——八项棘轮指标、`tests.unitFailures`、22 项 requestSize 全部持平，54 个声明的门全绿。这是本仓库 `gate` workflow 的首次通过。
...
## 2026-09-11/12 规格体系去重、拆分与归档

改了什么、为什么改见 `c697db2..41c3f5b` 共 19 次提交，此处只记排查过程与未覆盖范围。

- **核实方法**：删任何一段散文之前，先把它声称的规则提取关键词到全部 spec 文本里搜，确认已在正文才删。README 十段日期散文全部通过（环境叙述在 0001 §§3.3/7/12 与 0016 §8，窄修订在 0016 §7.2，schema 选择在 0015 §6.1 与 0016 §§7.2/10/12，0.4 重置在 0013 §0.4，故事预算在 0015 §§6.1/17），所以那次是纯去重。ADR 0015 的三条没通过——「单一 decision 填写面」「连续性审核只报相悖」「到期 ActorPlan 执行」在任何 SPEC 里都搜不到，是决策记录在当规格用，经用户同意才写成条款。
- **核实中查出的错**：索引长期引用 `SPEC 0016 §9.2`，但 §9 没有小节，该条款从不存在。
- **自己制造又修掉的三处**：(1) 拆分册时最后一个区间写成 `9-13`，`0016 §14` 落在所有区间之外被丢掉，条款数 34→33；(2) 日志按月拆分后条目里 `](agent/foo.md)` 深了一层，63 条链接失效，是写了 `check-doc-links` 才发现的；(3) `stale-gate` 两次误报——先是 frontmatter 批量提交、后是散文头部去重，都被日期检查当成正文变更，改成同时忽略 frontmatter 与 H1 下方元数据列表才回到真实值。
- **判断依据留档**：`gates_verified_on` 只在改动没有改变任何一句规则文本时可盖。0010/0012/0013 盖了（搬动与拆分可验证为保文本：条款数不变、标题逐条同序、无 unknown-clause 错误）；0015/0016 没盖，它们是 9 月 9 日改了规则而门没跟上，属真问题，改由补门清除。
- **技能触发率未验证，且测量本身失效**：`claude -p` 在本机与用户终端均返回 401（钥匙串有凭据但 token 过期）。`scripts/run_eval.py:88` 用 `stderr=subprocess.DEVNULL` 且从不检查退出码，于是每次调用失败都被记成「技能未触发」，产出 `precision=100% recall=0%` 这种看着成立的数字。20 条查询全部 `rate=0/3`、Test 集用时 `0.0s` 是识别它的线索。未据此修改描述。
- **未覆盖**：`docs/agent/proposals/vnext-fiction-time-contract-proposal.md` 引用被 `1357442` 删除的源码文件，是既有断链，登记基线未修。深 Module 边界 152 处与单测 98 个失败两项欠账未动，任务书在 `docs/agent/README.md`。剩余 6 条 oversize 全部超 1%–20%，未再拆。
...
## 2026-09-12 分组步骤 wire 回归定位与真实批次

- 原 Vitest 对比混用了 2 文件/57 测试与 65 文件/426 测试，导致“230 对 18”的误判。同范围复测定位出 8 个新增助手失败；多数 FORM_INVALID 在基线已存在。mock 断言和修订建票的裸 TypeError 都曾被包装成 timeout，Provider 完成日志可排除真实请求超时。
- round 120 已保存的 4 次回复暴露来源别名与读取集不一致，以及生产者扩表在后续 Rules 修订中丢失；对照使用相同回复、非空诊断和不同已证明类型集合定位。最后的终结预算拒绝用例先复现 3 次调用，再恢复到 2 次；这些过程没有靠重采样选成功。
- 未覆盖：round 121/122 的真实提交、重复提交与 replay 已核验，但 round 122 旁白重复前轮动作，固定组顺序仍限制跨类型晚事实；旧测试债务、全量最终 v66、v66 付费模型、生产发布及完整游玩没有验证。实际 API 用量、检查边界与证据路径见 [分组 wire 回执](../agent/receipts/vnext-grouped-steps-wire-validation.md)。
- 本地分支按用户授权继续原 worktree；整理基线为 e50194c、原 WIP aaedd46，无并行集成或冲突。外部动作只有有界真实 DeepSeek 调用；D1 migration 仅独立本地状态，无 push、部署、远端 migration 或资源变更。

## 2026-09-14 预览分支合并到本地 cloudflare

- 授权与基线：用户明确要求“合并到cloudflare”。目标起点 `7bad122`，共同祖先 `e5b2ddb`；预览起点 `5aaa62d`，本次先将已验证的 19 个文件保存为 `548eebd`，再合入 `claude/local-preview-changes-8206cb` 相对目标的全部 25 个提交。范围包含提案填写/纠错、执行顺序、旁白、线索板和归档恢复的前置改动；未作选择性摘取。
- 修改与冲突：产品代码与预览提交逐字一致；目标新增的五个测试文件头部规格说明保留。唯一内容冲突是本月日志末尾的独立追加，两边全文保留。目标原有 `AGENTS.md`、`CLAUDE.md`、`README.md`、`docs/agent/README.md`、`docs/agent/releases/release.md`、删除的 `handoff.md` 以及未跟踪的工作流回执，共七个路径逐文件校验不变，均不纳入合并提交。
- Node 组合验证：`npx tsx --test tests/knowledge-notebook.test.mjs tests/play-table-workspace-ui.test.mjs tests/kp-vnext-npc-recall.test.mjs tests/kp-vnext-narration.test.mjs tests/kp-vnext-proposal-revision.test.mjs tests/narration-provider-failure.test.mjs tests/kp-vnext-proposal-schema.test.mjs tests/kp-vnext-selection-amendment.test.mjs`，98/98，exit 0。`npm run typecheck`，exit 0。
- 归档验证：`npx vitest run tests/archive-do-resume-v2.test.ts -t 'backs off|uses the bound runtime'`，2 通过、1 失败、7 跳过，exit 1。两个新增退避用例覆盖失败恢复、驱逐与真实本地 D1 写入；旧 `uses the bound runtime` 用例在第 470 行仍得到 `pending=true`。在未合并的源提交 `548eebd` 单独运行同名用例，复现相同断言失败（exit 1），并与[先前归档基线说明](../agent/receipts/kp-preview-knowledge-and-retry-20260913.md)一致；未修改旧 fake D1 夹具或放松断言。
- 剩余限制：沿用[最新预览回执](../agent/receipts/kp-preview-naes-notebook-20260913.md)的验证边界；源分支已有三轮修订实现与 SPEC 0016 §7.2 一次修订约束的差异仍未处理，本次合并不构成规格合规确认。未追加真实模型探针、完整游玩、全量回归、生产构建、远端 migration、部署或 push。

## 2026-09-25 归档核对占满房间 CPU（生产事故）与第一步去哈希

- 现象：部署 `2657dd1c` 后，房间 9DCMQN（`room_22cb6874946c4b1d`，DO `81b5c263…`）15:02 开局，第 4 次提交后玩家连续看到 500（故障编号 `ZW-mugmgfvh-…`，`authorityTransient/authorityOverloaded`）。Workers Observability 的调用记录显示归档闹钟从 15:10 起每约 35 秒一次 `exceededCpu`/`exceededWallTime`，每次 CPU 26–32.5 秒；排在后面的 `jsrpc` 随对象重置一起失败。15:19–15:30 的 20 次闹钟全部超限，没有一次完成。这桌部署前没有任何调用记录，无法做前后对照。
- 测量（从 D1 读出这桌最近一次存入的归档：40 个事件、6 条动作记录、1.87 MB，本地 Node、部署源码 `61003c0`）：完整 `validateStoryArchive` 16.0 秒 CPU；一次从创世重放 1.6 秒；每条动作记录核对 0.5–4.8 秒。CPU 采样 15.8 秒里 13.1 秒在 `prefix` 从头重放，自用时间前几名是 `sha256Hex` 6.1 秒、`canonicalize` 2.5 秒、`assertCanonicalString` 1.5 秒。逐事件重放：检定的 `RandomnessRequested`、`ActivityCompletionInputRecorded` 各 160–300 毫秒，随世界状态从 150 KB 长到 300 KB 变贵，其余事件多在 30 毫秒内。线上 workerd 比本地约慢 2 倍多，一页（两遍完整重放加 3 条动作记录）因此超过 30 秒。
- 排除：部署带入的规则改动只让 NPC 视图在世界事实初始知识处多重建一次，采样约占 5%；检定分组（ADR 0041）是否让检定步骤的重放变贵没有对照测量。
- 处理：用户裁定去掉哈希，分三步。第一步按 [ADR 0054](../adr/0054-archives-are-copied-without-replay-checks.md) 去掉归档的重放核对：同一份归档的核对降到 48 毫秒。D1 投影审计不再写新行；Room 删除核对记号表。
- 未覆盖：规则里每一步的世界状态哈希仍在（第二步），房间加载与每次提交的 CPU 仍随世界状态增长；本次只在本地测量，线上效果要部署后看这桌的调用记录。

## 2026-09-25 第二步去哈希：事件、世界状态和重放

- 处理：按 [ADR 0055](../adr/0055-events-state-and-replay-carry-no-hashes.md) 去掉事件哈希链、世界状态哈希、作用域证明和重放时的重执行比对；D1 不再写逐条事件副本。同一份 9DCMQN 归档（40 个事件）从创世重放：1.6 秒降到约 0.14 秒 CPU（本地 Node，五次平均）。
- 排查中发现的依赖：更正审计记录里的事件载荷哈希不只是完整性记号。社交结算、代价前缀、骰子、反应回答、死亡与物件补全都靠它在同一根内找到此前提交的事件。只删事件的 `payloadHash` 时 Node 套件新增 189 个失败；改为审计记录保存载荷本身、按内容比较（同时去掉故事准入与旁白回执对旧哈希字段的读取）后降到 75 个，其余是测试夹具仍按旧字段构造。代价：9DCMQN 当前 12 条审计记录多 50 KB（状态 299 KB）。
- Worker 测试里的驱逐等待：`ability-operation.room` 的恢复用例在注入崩溃后调用 `evictDurableObject`，改动后每次卡满 30 秒超时，基线 2 毫秒完成。两边 Room 方法调用序列逐条相同；只有注入的异常从 RPC 抛出时才卡住，不抛异常时 2 毫秒；驱逐前在 DO 里分配一批临时对象促发垃圾回收，也降到 2 毫秒。结论：异常逸出 RPC 后，运行时要等垃圾回收才释放那次请求，优雅驱逐一直等它；基线每步都算整份状态的哈希，产生的垃圾恰好先触发了回收。该用例改用 `abortAllDurableObjects()` 模拟崩溃。其余 27 个使用驱逐的 Worker 测试文件本次都通过，未逐个检查是否依赖同样的回收时机。
- 验证：typecheck exit 0。Node 套件 1572/1576，失败的 4 个与部署基线逐名相同（`social-shape`、`private-lowering-diagnostics`）。Worker 全量 359/363，失败的 4 个与部署基线相同（`story-action.room` 3 个、`story-world-event.room` 1 个：测试替身从发给模型的上下文里读 `preparationHash`，而这份上下文在展示时去掉了全部服务器哈希）。用真实 9DCMQN 归档验证：旧头部的信封照常读取，40 个旧事件按记录折叠，之后新提交的事件不带哈希字段、父事件是最后一条旧事件，旧加新事件重放得到同一状态，续写的归档能再次读取。
- 未覆盖：只在本地测量；故事那 4 个基线失败未处理。第三步（冻结上下文、投影、Profile、幂等、骰子承诺、genesis 与归档内容哈希）未开始。
