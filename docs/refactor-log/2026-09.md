...
- 未解释的一项：`time-passage` 的「the private due capability rejects a changed root or Activity before any time is spent」本地 0.38s、预算 30000ms，余量 79 倍，超时解释不了。其真实断言内容不可得——见下条。
