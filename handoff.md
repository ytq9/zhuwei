...
**2026-09-09 晚追加（本地，未真实验证）：** 生产新房间一句提问即 `PROPOSAL_INPUT_BUDGET_EXCEEDED`，根因是冻结上下文先收集整个场景（全部可见 NPC 私有知识、开场实物全部事实、单字「的」命中别名）。已按 SPEC 0016 §4.3 改成按行动相关性构造并去掉模型视图里的版本 hash：真实黑橡房间点名提问的 offer 估算 44,825→25,730，四类表填写从被拦截到 41,549。本地定向证据与基线红的逐名对照见 [refactor-log](docs/refactor-log.md) 当日末条；**下一批真实 DeepSeek 批次须先复核 offer/填写的 `estimatedInputTokens` 与 usage**，58,000 开发准入上限未动。随后用户决定并已本地完成：类型依赖改软（authorItem 一族 schema 12.8k→8.6k）与持有知识分层加载（目录完整、正文按相关性）；选择阶段的动态召回例外暂缓讨论。**真实批次 round93/94（源码 `d18288a`）**：首句 offer 真实 27,811 / 27,941 token（改前本地复现估算 44,825），填写 34,383 / 34,528，只冻结被点名 NPC 的快照；round93 首句提交但回复旁白撞到跑批工具本地 7 次上限（生产无上限），round94 因模型把物品定义写进承诺主体、Rules 预检诊断无字段路径而一次修订后 needsKp。两批都没跑到第二、三句。见 [round93](docs/agent/vnext-round93-validation.md)、[round94](docs/agent/vnext-round94-validation.md)。随后按用户决定：承诺主体三处 `subjectRefs` 改为枚举，lowering 用 Rules 同一判定预检并带字段路径（`tests/kp-vnext-promise-subjects.test.mjs`）；round95 在该源码上重跑：填写返回空 `{}`、修订稿嵌套 JSON 多一个闭合符，needsKp；round96 关系变化引用了 NPC 看不到的隐藏事实，同类裸代码缺口，已把 `basisFactRefs`、`promiseeRef`、`authorityRefs` 一并补上枚举与路径预检；round97 又是空对象填写加修订稿嵌套 JSON 出错；随后把空对象回复改为重发原填写请求（合同里的“原稿重发”），round98 前两句提交（首句填写空对象后重发原请求成功；等待句提交），第三句因批次 20 分钟时限在会话被限流暂停时过期而未发；round99 同一提交首句把承诺 delivery 多写 kind 字段，诊断只指到 terms，修订猜错，needsKp；随后把承诺 terms 形状校验改为逐字段定位（`terms/delivery/kind` 列允许字段），round100 前两句直接提交，第三句（无人被点名、observe+social、填写请求估算 53,695）两次空对象填写 needsKp——「去账台看副本」在相关性冻结上下文上仍未覆盖；空对象是模型主动给的空调用（finish_reason=tool_calls、thinking 关闭），无阈值规律，下一步取决于用户对动态召回的讨论。

**2026-09-10 凌晨追加：** 上一条的相关性冻结之后，round98–100 又暴露出第二层问题：没有点名 NPC 的句子仍会把每一位在场 NPC 的决策视图和全部知识送给模型（round100 第三句「去账台看副本」填写请求估算 53,695，两次空对象，needsKp）。按用户要求做了选择阶段召回（`add92b1`）：所有可见 NPC 的视图与每位持有者的完整记忆照旧冻结核验，但模型只收到被点名的视图、当前话题够得着的记忆正文，以及其余记忆的 gist 目录（带 handle）；offer 工具新增 `requestedNpcRefs` 与 `requestedKnowledgeRefs`，补选按并集追加，票据与 Room 阶段证明按同一选择重算表面。

round102 是它的第一次真实批次：模型读了目录并按 handle 取走瓦罗两条相关记忆，无人点名的等待句 offer 从 39,789 降到 25,511、填写从 42,907 降到 33,398；瓦罗当面拒绝抄送（合法世界内拒绝，无承诺无计划），等待句在权威侧提交后客户端 120 秒超时中止（[回执](docs/agent/vnext-round102-validation.md)）。第三句在真实批次上仍未覆盖：round101、round103 因本地 dev server 编译过慢与归档重试饱和（82 个房间、769 MB、9 个房间持续重试）根本没建出房间。下一批先解决本地 state 膨胀，或换一个能在几十秒内编译的环境。
