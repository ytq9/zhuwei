...
| observe 单独成根（三张表） | **round91：sensoryEvidence + characterInferences 入正史，`knowledge:` 与 `module-opening` 引用合法** |
