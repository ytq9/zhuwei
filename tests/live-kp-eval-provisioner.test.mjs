...
} from "../tools/run-live-kp-eval.mjs";
import {
  LIVE_KP_EVAL_MODEL,
...
function publicEvaluationReport() {
  return {
    schemaVersion: "zhuwei-live-kp-eval-report-v2",
    status: "pass",
    target: {
      origin: "https://zhuwei.yinskyriver.workers.dev",
      modelId: LIVE_KP_EVAL_MODEL,
      modelProvider: LIVE_KP_EVAL_PROVIDER,
    },
    execution: {
      mode: "live",
      liveModelVerified: true,
...
test("provisions two distinct fighter seats, verifies the live report, and exactly reports cleanup", async () => {
  ...
  try {
    ...
    assert.equal(report.evaluation.target.modelProvider, LIVE_KP_EVAL_PROVIDER);
...
test("rejects a passing report unless it proves the exact V3 model, provider, and live mode", async () => {
  const mock = await provisioningServer();
  try {
    await assert.rejects(
      runProvisionedLiveKpEvaluation({
        ...
        evaluationRunner: async () => ({
          ...
          target: {
            ...publicEvaluationReport().target,
            modelId: "historical-model",
          },
        }),
      }),
      (error) => {
        assert.ok(error instanceof LiveKpProvisionError);
        assert.equal(error.code, LIVE_KP_CLI_ERROR_CODES.evaluationVerification);
        assert.equal(error.report.cleanup.room.status, "deleteRoomConfirmed");
        assert.equal(error.report.cleanup.authUsers.status, "deletedAndVerified");
        return true;
      },
    );
  } finally {
