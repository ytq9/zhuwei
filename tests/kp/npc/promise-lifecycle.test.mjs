...
import { promiseFixture, makePromiseInput, changePromiseInput, dueWork, objectBundle, ACTOR, NPC, SCENE } from '../../support/fixtures/vnext-promise-lifecycle.mjs';
...
// SPEC 0006 §6 / SPEC 0004 §6: an NPC's own promise work starts through `step`
// like a player action. Its start Receipt carries only the private plan row and
// the Activity; no Viewer claim exists yet, so the range must project as an
// empty frozen Claims result instead of failing projection integrity.
test('starting an NPC\'s promised delivery projects as a silent range for the player viewer', () => {
  const f = promiseFixture('silent-start'), seedRoot = `${f.rootActionId}:original`;
  ...
  const original = commit(f, f.state, seed.command.rulesInput);
  ...
  const promised = commit(f, original.state, makePromiseInput(f, original.state, { terms: {
    kind: 'result', subjectRefs: [NPC, originalItem.entryId], delivery: { sourceRef: originalItem.entryId, itemRef: null, quantity: 1, destinationKind: 'holder', destinationRef: ACTOR },
  } }));
  ...
  const input = npcWorkRulesInput(toolResponse(objectBundle({ destination: ACTOR, sourceRef: originalItem.entryId })), request, promised.state, f.profiles);
  const started = commit(f, promised.state, input);
  assert.deepEqual(started.events.map(e => e.eventType), ['ActivityStarted', 'NpcWorkStarted']);
  assert.equal(started.state.campaignRuntime.npcPlans[work.npcWork.planId].status, 'started');
  const range = { receiptId: started.receipt.receiptId, actorCharacterId: NPC, priorState: promised.state, events: started.events };
  const projected = f.runtime.project(f.profiles, started.state, f.viewer, { committedRange: range });
  assert.equal(projected.kind, 'projected', JSON.stringify(projected.rejection));
  assert.deepEqual(projected.renderableClaims.claims, []);
  assert.equal(projected.renderableClaims.receiptId, started.receipt.receiptId);
});
