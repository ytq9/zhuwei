...
import { proposalRecoveryBinding } from "../../../app/_runtime/lib/room/proposal-invocation-recovery";
...
it("explicit proposal replacement keeps unknown holds, accepts only late billing evidence and spends one source-wide recovery", async () => {
  // SPEC 0016 §7.2 / SPEC 0011 §3: a replacement is another physical call.
  const room = stub("proposal-recovery"), input = { ...binding("proposal", 4), invocationKey: "proposal:prepared:1" };
  const replacement = proposalRecoveryBinding(input);
  const identities = await withJournal(room, (journal, store) => {
    expect(journal.begin(replacement)).toMatchObject({ kind: "rejected" });
    const original = journal.begin(input);
    if (original.kind !== "ready") throw new Error("expected initial permit");
    expect(journal.recoverProposal(input, original.invocationId)).toMatchObject({ kind: "rejected" });
    journal.complete(input, { ...original, result: { kind: "unknown" } });
    const oldBudget = store.readBudget(input.source.budgetAccountId)!;
    const recovered = journal.recoverProposal(input, original.invocationId);
    if (recovered.kind !== "ready") throw new Error("expected recovery permit");
    expect(recovered.capability).not.toBe(original.capability);
    expect(recovered.providerRequest).toEqual(original.providerRequest);
    expect(store.readBudget(input.source.budgetAccountId)).toMatchObject({ spent: { calls: 2 }, held: {
      inputTokens: oldBudget.held.inputTokens + input.reservation.inputTokens,
      outputTokens: oldBudget.held.outputTokens + input.reservation.outputTokens,
    } });
    expect(journal.recoverProposal(input, original.invocationId)).toMatchObject({ kind: "rejected" });
    expect(journal.begin(replacement)).toMatchObject({ kind: "waiting", code: "STORY_INVOCATION_PENDING" });
    journal.complete(replacement, { ...recovered, result: { kind: "completed", response: { accepted: true },
      usage: { inputTokens: 30, outputTokens: 40, costMicros: 20 } } });
    expect(journal.complete(input, { ...original, result: { kind: "completed", response: { late: true },
      usage: { inputTokens: 10, outputTokens: 20, costMicros: 10 } } })).toEqual({ kind: "saved", eligible: false });
    expect(journal.begin(input)).toMatchObject({ kind: "rejected" });
    expect(journal.begin(replacement)).toMatchObject({ kind: "completed", response: { accepted: true } });
    expect(store.readBudget(input.source.budgetAccountId)).toMatchObject({ spent: { calls: 2, inputTokens: 40, outputTokens: 60 },
      held: { calls: 0, inputTokens: 0, outputTokens: 0 } });
    return { original, recovered };
  });
  await evictDurableObject(room);
  await withJournal(room, (journal, store) => {
    const another = { ...input, invocationKey: "proposal:different-submission:2" };
    const stage = journal.begin(another);
    if (stage.kind !== "ready") throw new Error("expected next stage");
    journal.complete(another, { ...stage, result: { kind: "unknown" } });
    expect(journal.recoverProposal(another, stage.invocationId)).toMatchObject({ kind: "rejected", code: "STORY_RETRY_EXHAUSTED" });
    expect(journal.begin(replacement)).toMatchObject({ kind: "completed", invocationId: identities.recovered.invocationId });
    expect(store.readBudget(input.source.budgetAccountId)?.spent.calls).toBe(3);
  });
});

it.each(["budget", "quarantine"])("blocks explicit proposal recovery at the %s boundary without releasing the original hold", async boundary => {
  const room = stub(`proposal-recovery-${boundary}`);
  const input = { ...binding("proposal", boundary === "budget" ? 1 : 4), invocationKey: "proposal:prepared:1" };
  await withJournal(room, (journal, store) => {
    const original = journal.begin(input);
    if (original.kind !== "ready") throw new Error("expected initial permit");
    journal.complete(input, { ...original, result: { kind: "unknown" } });
    const budget = store.readBudget(input.source.budgetAccountId);
    if (boundary === "quarantine") {
      ...
      if (archive.kind !== "available") throw new Error("expected archive");
      store.clearForRoomDeletion();
      expect(store.restoreArchiveSnapshot({ source, snapshot: archive.snapshot,
        quarantine: { invocationIds: [original.invocationId], sourceBudgetAccountIds: [input.source.budgetAccountId] } }).kind).toBe("restored");
    }
    expect(journal.recoverProposal(input, original.invocationId)).toMatchObject({ kind: "rejected",
      code: boundary === "budget" ? "STORY_BUDGET_EXHAUSTED" : "STORY_INVOCATION_UNKNOWN" });
    expect(journal.read(proposalRecoveryBinding(input))).toEqual({ kind: "missing" });
    expect(journal.read(input)).toMatchObject({ kind: "found", invocation: { status: "unknown", eligible: true } });
    expect(store.readBudget(input.source.budgetAccountId)).toEqual(budget);
  });
});
