...
import { storyNpcPendingRequest, freezeStoryNpcPendingContext } from '../../../app/_runtime/lib/room/story-npc-pending.ts';
...
import { pendingFixture, pendingStores, pendingArchive, pendingSnapshot, sourceOf, beginPending,
  completePending, freezePending, pendingResponse, ACTOR, TARGET } from '../../support/fixtures/story-npc-pending.mjs';
...
test('complete story archive envelope reads zero, completed and unknown pending hosts from real ledger snapshots', async () => {
  for (const outcome of ['zero', 'completed', 'unknown']) {
    ...
    const { head: _head, ...material } = context;
    const verified = await validateStoryArchive(await buildStoryArchive({ ...material, hostBindings, generation: '1' }));
