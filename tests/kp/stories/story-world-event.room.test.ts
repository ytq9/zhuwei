...
import { storyReviewBody, storyResponse } from "../../support/fixtures/story-creation.mjs";
import { initialize, draft, bundle, response, record, ALICE, ACTOR, SCENE, LIAN, FACT, PRIVATE, type Json } from "../../support/fixtures/story-action-room";
...
describe("committed world events through the actual Room host", () => {
  ...
  it("a real NPC due prepares a ready library story and the later player reuses it without more author calls", async () => {
    ...
    expect(saved.outcomes, JSON.stringify({ outcomes: saved.outcomes, calls: c.calls, error: c.error,
      checkpoints: saved.jobs.map(job => job.checkpoint) })).toMatchObject([{ kind: "ready" }]);
