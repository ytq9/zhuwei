...
for (const later of [false, true]) it(`production Room: compiled player card and noncombat spells leave the room ready for ${later ? "later materialization" : "joined character registration"}`, async () => {
  ...
  for (const spellId of ["cure", "healing-word"]) {
    ...
    const pending = record(await run(stub, { kind: "intent", submissionId: `submission:real-catalog:${spellId}`, text: `我对自己施放已经准备的${spellId}。` }, c));
    // SPEC 0003 §§7、8: confirm the owned frozen dice through Room; never
    // assume that submitting the intent has already spent slots or healed.
    expect(pending.kind).toBe("awaitingPlayerRoll");
    const result = await run(stub, { kind: "roll", submissionId: `submission:real-catalog:${spellId}:roll`,
      randomnessId: String((pending.pendingPlayerRolls as RecordValue[])[0].id) }, c);
