...
import { buildRoomTelemetryEvent, buildModelInvocationTelemetryEvent, failureCodeIsRetryable } from '../../../app/_runtime/lib/room/telemetry.ts';
...
function model(request, mode = 'success') {
  ...
  if (mode === 'presentation' || mode === 'uncertain') {
    const uncertain = mode === 'uncertain';
    report.checks[uncertain ? 'continuity' : 'presentation'] = uncertain ? 'uncertain' : 'fail';
    report.issues.push({ code: uncertain ? 'REVIEW_UNCERTAIN' : 'PRESENTATION', check: uncertain ? 'continuity' : 'presentation',
      constraintRef: 'policy:presentation', quote: BODY, occurrence: 0, reason: SECRET });
  }
  const adapter = createAuthoritativeKpAdapter({ onInvocationReceipt: receipt => receipts.push(receipt), ai: { async run(_model, input) {
    calls.push(input); assert.ok(calls.length <= 4, 'one bounded repair only');
    ...
    if (calls.length === 1 || calls.length === 3) return { choices: [{ message: { tool_calls: [{ type: 'function', function: {
      name: 'submit_frozen_narration', arguments: JSON.stringify(mode === 'malformed' ? { body: BODY, extra: SECRET } : { body: BODY }),
...
for (const [mode, code, callCount] of [
  ...
  ['grounding', 'NARRATION_GROUNDING_REJECTED', 4],
  ['uncertain', 'NARRATION_REVIEW_UNCERTAIN', 2],
  ...
  if (mode.endsWith('Rejected') || mode === 'rateLimited') {
    const event = buildModelInvocationTelemetryEvent({ receipt: run.receipts.at(-1) });
    assert.equal(event.failureReason, mode === 'rateLimited' ? 'providerRateLimited' : 'providerRequestInvalid');
    assert.equal(event.failureStage, 'modelRequest');
    assert.equal(event.providerStatus, mode === 'rateLimited' ? 429 : 400);
  }
...
// SPEC 0011 §1 / SPEC 0016 §8.3: a language judgment or uncertainty is not proof of a factual conflict.
test('presentation and uncertain reviews explain the actual failure and frozen recovery limit', () => {
  for (const [code, reason] of [['NARRATION_PRESENTATION_REJECTED', /表达检查/],
    ['NARRATION_REVIEW_UNCERTAIN', /无法确认/]]) {
    const copy = publicNarrationRecoveryReason('rejected', code);
    assert.match(copy, reason);
    assert.match(copy, /无法通过重试恢复/);
    assert.doesNotMatch(copy, /事实不一致|请点击/);
  }
  assert.doesNotMatch(publicNarrationFailureReason('NARRATION_GROUNDING_REJECTED'), /事实不一致/);
});
