...
it.each([DEFAULT_KP_MODEL, GPT_6_LUNA_MODEL].flatMap(modelId => [1, 2, 3].map(stage => ({ modelId, stage }))))(
  ...
  const success: Provider = async request => sentToolName(request) === OFFER_KP_PROPOSAL_BUNDLE_TOOL_NAME
    ...
    : toolResponse(proposal(stage === 3 && sentToolName(request) === SUBMIT_KP_PROPOSAL_BUNDLE_TOOL_NAME ? "" : "打开控制件。"));
  await run(stub, input, capture, async (request, target, captured) => {
    if (capture.starts.at(-1)!.request.ordinal === stage) throw new Error("simulated transport timeout");
    return success(request, target, captured);
  });
  const original = capture.starts.at(-1)!;
  expect(original.result.kind).toBe("ready");
  expect((await snapshot(stub)).events).toEqual(before.events);
  const recovery = { ...retry(capture, input), recoverProposal: true };
  await evictDurableObject(stub);
  const outcome = await run(stub, recovery, capture, async (request, target, captured) => {
    const active = capture.starts.at(-1)!;
    const id = String(capture.prepared!.preparedActionId);
    if (active.request.ordinal === stage) {
      expect(request).toEqual(original.request.request);
      expect(await target.beginVNextProposalInvocation(ALICE, id, active.request))
        .toMatchObject({ kind: "retryableFailure", code: "STORY_INVOCATION_PENDING" });
      expect(await target.completeVNextProposalInvocation(ALICE, id, {
        ordinal: original.request.ordinal, requestHash: original.request.requestHash,
        capability: original.result.kind === "ready" ? original.result.capability : "",
        result: { kind: "completed", response: toolResponse({ kind: "schemaRequest", capabilities: ["inWorldRefusal"] }) },
      })).toMatchObject({ kind: "rejected", code: "PROPOSAL_INVOCATION_SUPERSEDED" });
    }
    return success(request, target, captured);
  });
  ...
  const ledger = await runInDurableObject(stub, (_instance, ctx) => ctx.storage.sql.exec<{
    eligible: number; status: string; held_json: string;
  }>("SELECT eligible, status, held_json FROM story_creation_invocations WHERE purpose = 'proposal' ORDER BY rowid").toArray());
  expect(ledger).toHaveLength(Math.max(2, stage) + 1);
  expect(ledger[stage - 1]).toMatchObject({ eligible: 0, status: "completed" });
  expect(JSON.parse(ledger[stage - 1]!.held_json).inputTokens).toBeGreaterThan(0);
...
it("stops explicit proposal recovery after one replacement timeout, including eviction", async () => {
  ...
  const capture: Capture = { starts: [], providerRequests: [] };
  const timeout: Provider = async () => { throw new Error("controlled timeout"); };
  expect(await run(stub, input, capture, timeout)).toMatchObject({ code: "PROPOSAL_RECOVERY_REQUIRED" });
  expect(await run(stub, { ...input, recoverProposal: true }, capture, timeout))
    .toMatchObject({ kind: "rejected", code: "PROPOSAL_RECOVERY_EXHAUSTED", action: "notCommitted" });
  expect(capture.providerRequests).toHaveLength(2);
  const frozen = await snapshot(stub, capture);
  await evictDurableObject(stub);
  expect(await run(stub, { ...input, recoverProposal: true }, capture, timeout))
    .toMatchObject({ kind: "rejected", code: "PROPOSAL_RECOVERY_EXHAUSTED", action: "notCommitted" });
  expect(capture.providerRequests).toHaveLength(2);
  expect(await snapshot(stub, capture)).toEqual(frozen);
});

it.each(["frozen", "scope", "foreign"])("rejects explicit proposal recovery across the %s authority boundary", async boundary => {
  const stub = await initialize(`provider-recovery-boundary-${boundary}`);
  const input = action(`submission:recovery-boundary:${boundary}`), capture: Capture = { starts: [], providerRequests: [] };
  await run(stub, input, capture, async () => { throw new Error("controlled timeout"); });
  const request = { ...capture.starts[0]!.request, recoverUnknown: true as const };
  const id = String(capture.prepared!.preparedActionId);
  await runInDurableObject(stub, async (instance, ctx) => {
    if (boundary === "frozen") ctx.storage.sql.exec("UPDATE authority_submissions SET proposal_hash = ? WHERE prepared_action_id = ?", canonicalHash({ frozen: true }), id);
    if (boundary === "scope") ctx.storage.sql.exec("UPDATE authority_submissions SET prepared_scope_version = prepared_scope_version - 1 WHERE prepared_action_id = ?", id);
    const target = instance as unknown as Internals;
    expect(await target.beginVNextProposalInvocation(boundary === "foreign" ? BOB : ALICE, id, request))
      .toMatchObject({ kind: "rejected", code: boundary === "foreign" ? "PROPOSAL_REFERENCE_INVALID" : "PROPOSAL_RECOVERY_UNAVAILABLE" });
    expect(ctx.storage.sql.exec<{ count: number }>("SELECT COUNT(*) AS count FROM story_creation_invocations WHERE purpose = 'proposal'").one().count).toBe(1);
  });
});

it("explicit recovery settles an expired dispatch, but reuses an original reply that arrives before recovery", async () => {
  for (const lateReply of [false, true]) {
    const stub = await initialize(`provider-recovery-expired-${lateReply}`);
    const input = action(`submission:recovery-expired:${lateReply}`), capture: Capture = { starts: [], providerRequests: [] };
    await run(stub, input, capture, async () => { throw new Error("controlled disconnect"); });
    const original = capture.starts[0]!;
    const selection = toolResponse({ kind: "schemaRequest", capabilities: ["worldInteraction"] });
    await runInDurableObject(stub, async (instance, ctx) => {
      const target = instance as unknown as Internals, id = String(capture.prepared!.preparedActionId);
      if (lateReply) {
        expect(await target.completeVNextProposalInvocation(ALICE, id, { ordinal: 1,
          requestHash: original.request.requestHash, capability: original.result.kind === "ready" ? original.result.capability : "",
          result: { kind: "completed", response: selection } })).toMatchObject({ kind: "saved" });
      } else {
        // Emulate eviction after sending but before the outcome was journaled.
        ctx.storage.sql.exec("UPDATE story_creation_invocations SET status = 'started', completed_at = NULL, lease_until = 0, completion_hash = NULL WHERE invocation_id = ?",
          target.vnextInvocation(id, 1)!.invocation_id);
      }
    });
    await evictDurableObject(stub);
    expect(await run(stub, { ...input, recoverProposal: true }, capture, async request =>
      sentToolName(request) === OFFER_KP_PROPOSAL_BUNDLE_TOOL_NAME ? selection : toolResponse(proposal())))
      .toMatchObject({ kind: "committed" });
    expect(capture.providerRequests).toHaveLength(lateReply ? 2 : 3);
  }
});
...
describe("vNext Provider invocation and Room persistence", () => {
  ...
  it("preserves the frozen repair proof after an unknown dispatch and never automatically resends it after eviction", async () => {
    ...
    expect(capture.starts.at(-1)!.result).toMatchObject({ kind: "retryableFailure", code: "PROPOSAL_RECOVERY_REQUIRED" });
    expect(capture.providerRequests).toHaveLength(3);
    expect(await snapshot(stub, capture)).toEqual(waiting);
    await run(stub, retry(capture, input), capture, provider);
    expect(capture.providerRequests).toHaveLength(3);
  ...
  it("an ambiguous 429 preserves an unknown dispatch across eviction and never automatically resends after expiry", async () => {
    const stub = await initialize("provider-room-rate-limit-deadline");
    const capture: Capture = { starts: [], providerRequests: [], invocationEvents: [] };
    ...
    expect(delayed, JSON.stringify(delayed)).toMatchObject({ kind: "retryableFailure", action: "notCommitted",
      code: "PROPOSAL_RECOVERY_REQUIRED", retryAfter: 2 });
    expect(capture.invocationEvents).toContainEqual(expect.objectContaining({
      failureDiagnostic: { reason: "providerRateLimited", stage: "modelRequest", retryability: "retryable", providerStatus: 429 },
    }));
    const waiting = await snapshot(stub, capture);
    expect(waiting.state).toEqual(before.state);
    expect(waiting.events).toEqual(before.events);
    ...
    expect(capture.starts.at(-1)!.result).toMatchObject({ kind: "retryableFailure", code: "PROPOSAL_RECOVERY_REQUIRED" });
    expect(capture.invocationEvents).toContainEqual(expect.objectContaining({
      failureDiagnostic: { reason: "invocationOutcomeUnknown", stage: "invocationJournal", retryability: "blocked" },
    }));
    expect(capture.providerRequests).toHaveLength(1);
    expect(await snapshot(stub, capture)).toEqual(waiting);
