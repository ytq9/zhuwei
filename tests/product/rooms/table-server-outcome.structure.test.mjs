...
test("terminal predecessor rejection does not advertise a retry or refresh for the new action", async () => {
  // SPEC 0015 §8.2: preserve the authority verdict through the HTTP DTO.
  const mapOutcome = await tableOutcomeMapper();
  const result = mapOutcome("submission:blocked-by-old-reply", {
    kind: "rejected", code: "narrationPredecessorBlocked",
    action: "notCommitted", narration: "notApplicable",
    explanation: "PRIVATE_OLD_NARRATION",
  }, true);
  assert.equal(result.action, "notCommitted");
  assert.equal(result.outcomeKind, "rejected");
  assert.notEqual(result.retryable, true);
  assert.match(result.error, /前一条已结算行动.*无法通过重试恢复/);
  assert.doesNotMatch(result.error, /请刷新|请点击|重试 KP 回复|PRIVATE_/);
});
