 ...
 * tests/product/multiplayer/multiplayer.room.test.ts; §4 concurrent intent lives there, because
...
test("service-authoritative membership, Seat, control, host, and pending revocation share one event path", () => {
  ...
  assert.deepEqual(joined.result.events.map(({ eventType }) => eventType), [
    "MemberJoined",
    "SeatGranted",
    // SPEC 0007 §2: registration precedes granting the new character's control.
...
test("character materialization, semantic gear changes, and host departure stay on authoritative events", () => {
  ...
  const controlIndex = materializedTypes.indexOf("CharacterControlGranted");
  assert.ok(controlIndex > 0);
  assert.ok(materializedTypes.slice(0, controlIndex).every(type => type === "DefinitionRegistered"));
  assert.equal(materializedTypes.filter(type => type === "CharacterControlGranted").length, 1);
