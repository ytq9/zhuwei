...
import { diagnosticReferenceFromMessage } from "../../../app/_runtime/lib/platform/diagnostic-reference.ts";
...
test("player error feedback preserves public explanations and adds guidance for HTTP failures", async () => {
  ...
  try {
    ...
    const result = await fetchTable({ data: "ERRORS" });
    const reference = diagnosticReferenceFromMessage(result.error);
    assert.ok(reference, "business errors include a searchable request reference");
    assert.deepEqual(result, { ...rejection, error: `${rejection.error}\n故障编号：${reference}` },
      "business failures retain their server-owned status and explanation before the diagnostic suffix");
  } finally { globalThis.fetch = original; }
});
