...
describe("Room DO authoritative randomness crash recovery", () => {
  ...
  it("normalizes a legacy plain batch fulfillment and reuses its persisted candidate", async () => {
    const room = await initializedRoom("legacy-plain-batch-candidate");
    ...
    await expect(runInDurableObject(harnessStub, async (instance) => {
      ...
      target.authorityRecoveryCheckpoint = (name: string) => {
        if (name === "afterRandomnessCandidateCommit") {
          throw new Error("simulated-crash:legacy-plain-batch-candidate");
        }
      };
      return target.commit(ALICE, room.preparedActionId, structuredClone(room.proposal));
    })).rejects.toThrow("simulated-crash:legacy-plain-batch-candidate");

    const persistedCandidates = await runInDurableObject(
      harnessStub,
      async (_instance, state) => {
        const row = state.storage.sql.exec<{
          ...
          SELECT fulfillment_json, candidates_json, status
          FROM authority_randomness_batches
          WHERE prepared_action_id = ?
        `, room.preparedActionId).toArray()[0];
        expect(row?.status).toBe("candidateCommitted");
        expect(row?.candidates_json).not.toBeNull();
        const envelope = record(JSON.parse(row!.fulfillment_json), "multi-wave envelope");
        const waves = list(envelope.waves, "multi-wave envelope waves");
        expect(waves).toHaveLength(1);
        const legacyFulfillment = record(waves[0], "legacy wave").fulfillment;
        state.storage.transactionSync(() => {
          state.storage.sql.exec(
            `UPDATE authority_randomness_batches
             SET fulfillment_json = ?
             WHERE prepared_action_id = ?`,
            JSON.stringify(legacyFulfillment),
            room.preparedActionId,
          );
        });
        return JSON.parse(row!.candidates_json!) as unknown[];
      },
    );
    const persistedCandidate = record(persistedCandidates[0], "persisted legacy candidate");
    ...
    const recovered = record(await room.stub.commit(
      ...
      room.preparedActionId,
      structuredClone(room.proposal),
    ), "legacy plain batch recovery");
    expect(recovered.kind, JSON.stringify(recovered)).toBe("committed");
    expect(randomResult(recovered).faces).toEqual(persistedCandidate.faces);

    const retry = record(await room.stub.commit(
      ALICE,
      room.preparedActionId,
      structuredClone(room.proposal),
    ), "legacy plain response-lost retry");
    expect(retry.receipt).toEqual(recovered.receipt);
    expect(randomResult(retry).faces).toEqual(persistedCandidate.faces);
  });

  it("lazily promotes a legacy candidate-committed journal without rerolling", async () => {
    const room = await initializedRoom("legacy-single-journal-candidate");
    const harnessStub = room.stub as never;
    await expect(runInDurableObject(harnessStub, async (instance) => {
      const target = instance as unknown as {
        authorityRecoveryCheckpoint?: (name: string) => void;
      ...
      target.authorityRecoveryCheckpoint = (name: string) => {
        if (name === "afterRandomnessCandidateCommit") {
          throw new Error("simulated-crash:legacy-single-journal-candidate");
        }
      };
      return target.commit(ALICE, room.preparedActionId, structuredClone(room.proposal));
    })).rejects.toThrow("simulated-crash:legacy-single-journal-candidate");

    const legacyFaces = await runInDurableObject(harnessStub, async (_instance, state) => {
      const batch = state.storage.sql.exec<{
        ...
        SELECT proposal_hash, requests_json, fulfillment_json,
               request_events_json, answered_pending_input_id,
               candidates_json, status
        FROM authority_randomness_batches
        WHERE prepared_action_id = ?
      `, room.preparedActionId).toArray()[0];
      expect(batch?.status).toBe("candidateCommitted");
      const requests = list(JSON.parse(batch!.requests_json), "legacy source requests");
      const candidates = list(JSON.parse(batch!.candidates_json!), "legacy source candidates");
      const envelope = record(JSON.parse(batch!.fulfillment_json), "legacy source envelope");
      const waves = list(envelope.waves, "legacy source waves");
      expect(requests).toHaveLength(1);
      expect(candidates).toHaveLength(1);
      expect(waves).toHaveLength(1);
      const request = record(requests[0], "legacy source request");
      const candidate = record(candidates[0], "legacy source candidate");
      const fulfillment = record(record(waves[0], "legacy source wave").fulfillment, "legacy fulfillment");
      expect(fulfillment.kind).toBe("singleContinuation");
      state.storage.transactionSync(() => {
        state.storage.sql.exec(
          `INSERT INTO authority_randomness_journal (
           ...
           ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'candidateCommitted')`,
          room.preparedActionId,
          String(request.randomnessId),
          batch!.proposal_hash,
          String(request.requestHash),
          String(request.frozenParametersHash),
          JSON.stringify(request.request),
          JSON.stringify(fulfillment.continuation),
          batch!.request_events_json,
          batch!.answered_pending_input_id,
          JSON.stringify(candidate.faces),
        );
        state.storage.sql.exec(
          "DELETE FROM authority_randomness_batches WHERE prepared_action_id = ?",
          room.preparedActionId,
        );
      });
      expect(state.storage.sql.exec<{ total: number }>(`
        SELECT COUNT(*) AS total FROM authority_randomness_batches
        WHERE prepared_action_id = ?
      `, room.preparedActionId).toArray()[0]?.total).toBe(0);
      return structuredClone(candidate.faces);
    });
    ...
    expect(recovered.kind, JSON.stringify(recovered)).toBe("committed");
    expect(randomResult(recovered).faces).toEqual(legacyFaces);

    const promoted = await runInDurableObject(harnessStub, async (_instance, state) =>
      state.storage.sql.exec<{ candidates_json: string | null; status: string }>(`
        SELECT candidates_json, status FROM authority_randomness_batches
        WHERE prepared_action_id = ?
      `, room.preparedActionId).toArray()[0]);
    expect(promoted?.status).toBe("finalized");
    const promotedCandidate = record(
      list(JSON.parse(promoted!.candidates_json!), "promoted candidates")[0],
      "promoted candidate",
    );
    expect(promotedCandidate.faces).toEqual(legacyFaces);
  });
