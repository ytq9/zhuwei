...
test("proposal recovery is explicit, keeps the original identity, and removes the button when exhausted", async () => {
  // SPEC 0016 §7.2: the public classification and the actual component agree.
  const restoreStorage = installSessionStorage(memoryStorage());
  const originalFetch = globalThis.fetch, previousActEnvironment = globalThis.IS_REACT_ACT_ENVIRONMENT;
  globalThis.IS_REACT_ACT_ENVIRONMENT = true;
  const calls = [];
  globalThis.fetch = async (_url, init) => {
    const request = JSON.parse(String(init?.body));
    calls.push(request.data);
    const code = calls.length === 1 ? "PROPOSAL_RECOVERY_REQUIRED" : "PROPOSAL_RECOVERY_EXHAUSTED";
    return Response.json({ action: "notCommitted", narration: "notApplicable", code,
      retryable: failureCodeIsRetryable(code), error: publicAuthoritativeOutcomeError({ kind: "retryableFailure", code }) });
  };
  let renderer, client;
  try {
    ...
    await act(async () => { renderer = create(createElement(QueryClientProvider, { client },
      createElement(PlayTable, { code: "TACTIC", snap: playableSnap(compileSheet) }))); });
    await act(async () => { renderer.root.findByType("textarea").props.onChange({ target: { value: "我检查门缝。" } }); });
    await act(async () => {
      renderer.root.findByType("form").findAllByType("button").at(-1).props.onClick();
      await new Promise(resolve => setTimeout(resolve, 0));
    });
    assert.equal(calls.length, 1); assert.equal(calls[0].recoverProposal, undefined);
    await act(async () => {
      renderer.root.findByProps({ "data-action-recovery-submit": true }).props.onClick();
      await new Promise(resolve => setTimeout(resolve, 0));
    });
    assert.equal(calls.length, 2);
    assert.deepEqual(calls[1], { ...calls[0], recoverProposal: true });
    assert.equal(renderer.root.findAllByProps({ "data-action-recovery-submit": true }).length, 0);
    assert.equal(renderer.root.findByType("textarea").props.value, "我检查门缝。");
    assert.match(JSON.stringify(renderer.toJSON()), /恢复次数已用完/);
    assert.equal(failureCodeIsRetryable("PROPOSAL_RECOVERY_UNAVAILABLE"), false);
  } finally {
    const { act } = await import("react-test-renderer");
    if (renderer) await act(async () => renderer.unmount());
    client?.clear(); restoreStorage(); globalThis.fetch = originalFetch;
...
test("stable transport cache wins over a freshly requested id until a terminal result", async () => {
  ...
  await callWithStableSubmission({
    command: "sendAction",
    data: { ...data, submissionId: "submission:fresh-but-wrong", recoverProposal: true },
    storage,
