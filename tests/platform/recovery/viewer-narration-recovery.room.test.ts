...
describe("V3 viewer-local narration recovery", () => {
  it("withholds a newly committed clue until its grounded KP reply is delivered", async () => {
    ...
    // SPEC 0005 §7、SPEC 0010 §7: the delivered knowledge stays exact even
    // when the presentation groups untitled entries into a notebook card.
    expect(recoveredTable.clues.flatMap(clue => clue.notes ?? [clue])).toEqual([expect.objectContaining({
      id: knowledgeRef,
      text: clueText,
    })]);
  });
