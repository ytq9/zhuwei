...
describe("Room DO incremental D1 archive continuation", () => {
  ...
  it.each([
    ...
    const failed = await runInDurableObject(stub as never, async instance => {
      ...
      const error = vi.spyOn(console, "error").mockImplementation((...args: unknown[]) => { captured.push(args); });
      try { await target.flushAuthoritativeD1ArchivePage(); } finally { error.mockRestore(); }
      const now = Date.now();
      const progress = target.authorityStore.archiveProgress();
      await target.resumeAuthoritativeD1Archive();
      await target.resumeAuthoritativeD1Archive();
      return { progress, now, attempts, telemetry: capturedTelemetry(captured) };
    });
    ...
    // SPEC 0011 §5: the actual archive failure identifies its stage while
