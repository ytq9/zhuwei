...
// SPEC 0011 §5: archive diagnostics retain only a closed stage/code pair.
test("archive failures identify their stage without exposing exception content", async () => {
  const input = { eventName: "room.archive.failed", failure: { code: "ARCHIVE_APPEND_FAILED" },
    archive: { status: "failed", failureStage: "buildEnvelope", error: new TypeError("STORY_ARCHIVE_HOST_BINDING_INVALID") } };
  const known = await buildRoomTelemetryEvent(input);
  assert.equal(known.archiveFailureStage, "buildEnvelope");
  assert.equal(known.archiveFailureCode, "STORY_ARCHIVE_HOST_BINDING_INVALID");
  assert.equal(known.errorCode, "archiveFailure");
  for (const message of [SENSITIVE.prompt, `STORY_ARCHIVE_HOST_BINDING_INVALID:${SENSITIVE.truth}`]) {
    const unknown = await buildRoomTelemetryEvent({ ...input, archive: { ...input.archive, error: new Error(message) } });
    assert.equal(unknown.archiveFailureCode, "unclassified");
    assert.doesNotMatch(JSON.stringify(unknown), /SECRET_SENTINEL| at /);
  }
  const forgedStage = await buildRoomTelemetryEvent({ ...input, archive: { ...input.archive, failureStage: SENSITIVE.prompt } });
  assert.equal(forgedStage.archiveFailureStage, undefined);
  assert.equal(forgedStage.archiveFailureCode, undefined);
  const unrelated = await buildRoomTelemetryEvent({ ...input, failure: { code: "AI_TIMEOUT" } });
  assert.equal(unrelated.archiveFailureStage, undefined);
});
...
test("party narration emits its own redacted model invocation receipt", async () => {
  ...
  const narrationStart = party.indexOf("const narration = createRoomKpAdapter");
