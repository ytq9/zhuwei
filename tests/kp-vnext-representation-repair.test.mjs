...
function interaction() {
  return { schema: VNEXT2_PROPOSAL_BUNDLE_SCHEMA, kind: 'proposalBundle', mode: 'adjudication', basisRefs: ['sceneFeature:chain'],
    adjudication: { kind: 'directSuccess', durationMicros: '300000000', risk: '没有显著风险。', successOutcome: '能够检查目标。' }, terminal: null,
...
test('semantic, authority and unknown-field errors prevent every otherwise safe partial repair', () => {
  for (const mutate of [
    ...
    bundle => { bundle.adjudication = { kind: 'check', durationMicros: '300000000', checkKind: 'abilityCheck', ability: 'wis', skill: null, dc: '12', mode: 'normal', risk: '失败风险。', successOutcome: '成功。', failureOutcome: '失败。' }; },
  ]) {
  ...
  check.adjudication = { kind: 'check', durationMicros: '300000000', checkKind: 'abilityCheck', ability: 'wis', skill: null, dc: 12, mode: 'normal', risk: '失败风险。', successOutcome: '成功。', failureOutcome: '失败。' };
...
test('a complete shared check retains its DC, risk and both consequences during prose normalization', () => {
  ...
  expected.adjudication = { kind: 'check', durationMicros: '300000000', checkKind: 'abilityCheck', ability: 'wis', skill: null, dc: 12, mode: 'normal',
