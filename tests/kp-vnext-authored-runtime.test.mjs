...
function begin(name,value){const f=createAuthoredProbeFixture(name);const lowered=lowerVNext2ProposalBundle({value,rootActionId:f.rootActionId,actorCharacterId:ACTOR,requiredContext:f.requiredContext,state:f.state});assert.equal(lowered.kind,'accepted',JSON.stringify(lowered));const r=stepActionToDecision(f.runtime, f.profiles,f.state,lowered.command.rulesInput);return {f,r,input:lowered.command.rulesInput};}
function settle(f,r,face=2){assert.equal(r.kind,'awaitingRandomness',JSON.stringify(r));assert.deepEqual(r.events.map(event=>event.eventType),['ActivityStarted','FictionTimeAdvanced','RandomnessRequested'],'only committed time and the frozen dice request precede authored effects');const done=f.runtime.step(f.profiles,r.state,{kind:'fulfillAuthoritativeRandomness',continuation:r.continuation,rolls:r.randomnessRequest.dice.flatMap(d=>Array(Number(d.count)).fill(Math.min(face,Number(d.sides))))});assert.equal(done.kind,'committed',JSON.stringify(done));const replay=f.runtime.replay(f.genesis,[...r.events,...done.events]);assert.equal(replay.kind,'replayed',JSON.stringify(replay));assert.deepEqual(replay.state,done.state);return done;}
