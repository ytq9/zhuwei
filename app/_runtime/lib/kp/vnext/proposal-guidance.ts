...
依据冻结RequiredContext的正文、授权、版本和引用：known须保留记录类型与状态，来源主张不自动为真，scheduled计划/承诺/未来trace不是已发生证据；knownAbsent只证明带版本的局部范围；openBlank是创作权限而非存在证明；重大歧义不能擅选危险解释；unavailable是技术缺失，不猜测或包装成世界拒绝。
...
只填当前kind分支及嵌套对象声明的字段，不从其他分支或Context复制字段。引用从对应冻结候选选实际支持记录；开放授权/局部不存在条目用其列出的支持引用。精确复用本束新对象的局部名称；目标不自动成为内容证据，知识、感官和推断按表单选来源。
输出最小完整提案，必填字段齐全，空值按各字段schema的none哨兵，可空引用用{kind:"none"}，不省略或用空字符串。状态变化写合法操作或entries中的recordKind=effects，感知写recordKind=sensoryEvidence。summary、risk、successOutcome/failureOutcome只概括骰前分支，不创建事实或充当最终旁白；用自然明确且有依据的中文，不暗增陈设、因果、发现或奖励。不支持的机械诚实失败。`;
...
semanticKind=location时，definition.sceneRef填当前授权场景，geometry为新地点完整几何，observableState/affordances=none、mechanicDefinitionRefs=[]，地点ID由服务器派生。passage两端用授权scene或同束location handle，observableState限open/closed/blocked，affordances=none、mechanicDefinitionRefs=[]；traversal/travelDurationMicros定义通行方法/耗时。创建地点或连接不移动角色、不扣通行时间；未进入时只写当前可感知连接，不揭露目的地内部。`,
  observe: `inquiry写问题，method写观察或推断方法。focusRefs/subjectRef选实际感知主体，可用已列当前sceneRef做整体观察；多人/多物证据分别归属，无单独主体时subjectRef用none。知识、目录、Profile只作existingFactRefs/basisRefs，不充当空间目标。仅据已有知识推断时focusRefs=[]且不填感官证据；observe的时长仍按decision.duration，纯回顾用knowledgeReview。
直接感知用recordKind=sensoryEvidence；原因、时间、动机等解释用characterInferences，保留conclusion/confidence/evidence。heldKnowledge选本人已有knowledgeRef；sensoryEvidence/index按本分支sensoryEvidence子序列从0计数，observerRef必须是行动者。推断不强制玩家相信、不成为真相。未知内容获授权才先固化，对象存在不证明证据内容。`,
...
台词与后果必须一致：NPC在responseText里实际答应将来做事或持续遵守约束时，同分支consequences必须含对应promise，完整登记原约、期限与terms；不能让台词答应交付而consequences=[]。正式称作承诺不是前提，按该情境中话语的实际意思判断。明确拒绝、尚未答应、预测或转述不记新承诺。口头答应不代替实物执行，不能在只有social步骤时叙述已制作或已递交。
...
承诺按promisor选actor/npc，promiseeRef选实际受诺听众。NPC只约束自己，authorityRefs只填该npcRef；actor仅记录玩家本次明确承诺，content精确保留原表达，authorityRefs仅含actorRef、nextStep=none。接受条件、预测或转述不自动成为玩家承诺或付款。due是约定期限，与条件、计划及工期分开。terms内必须完整填写五个字段：kind、subjectRefs、delivery、parts、activation，后两者不能放在terms外。terms.kind=result/attempt/ongoing，terms.subjectRefs绑定主体和对象。承诺制作、复制或交付物品时必须填terms.delivery：未来物品尚未存在用itemRef=none，没有复制原件用sourceRef=none，仍填写数量和实际交付人物/地点；只有非物品义务才将整个delivery填none。terms.parts列需独立跟踪的额外部分，无则[]；terms.activation写真实生效条件，无则none。NPC答应采取行动时，nextStep写其紧接着要做的工作或决定；没有执行动作的持续约束用none。执行时再在本人知识内冻结做法、工期及效果，Activity完成才落地。
改约用promiseChange绑定原promiseRef/revision；expressionSource选actor/npc，expressionQuote精确复用玩家原表达或本分支responseText。KP按原约、情境及依据判断change是否成立、影响范围和剩余义务，不设统一双方审批；不能编造玩家新义务、抹去历史违约或把内部改计划当改约。仅真实传达有效变更才disclose=true。承诺及台词不提前执行开门、交付、战斗，痕迹或自报不等于履约。
初次交谈或没有既存失败记录时，retryChange必须为{kind:"none"}；不能用当前submissionId或玩家发言充当priorThreadRef。同一失败目标须addressedThreadRef及方法、具体条件或局势的实质变化，换措辞不能重骰。交谈时长由decision.duration冻结；后续等待或额外成本需独立可执行计划，不能只写risk/summary。`,
  worldInteraction: `操作已有或同束新对象。directTargetRefs非空，列实际操作对象；instrumentRefs只列工具，otherTargetRefs列其余实际受影响对象，无则[]。独立观察/推断用observe。感官证据的observerRef是感知者，subjectRef是被感知对象，保留布局、数量、状态和以英寸计的Geometry。
...
export const VNEXT_PROPOSAL_GUIDANCE_POLICY = deepFreeze({
  version: "zhuwei.proposal-guidance/v13", selection: "flat-type-selection-with-exact-terminal-and-step-surface/v4",
  storySelection: STORY_SELECTION_POLICY_HASH, selectionAuthority, terminalSelectionDescriptions, terminalFilling, authority, planRuling, sharedRuling, terminalRuling, filling, stages, recoveryInstructions, catalog: VNEXT_PROPOSAL_CAPABILITIES, producerContract: VNEXT_PROPOSAL_PRODUCER_CONTRACT,
...
export function vnextProposalSystemPrompt(stage: VNextProposalStage,
  ...
  return [authority, amendable ? stages.amendableProposal : stages.expandedProposal,
    ...(hasSteps ? [planRuling] : []),
