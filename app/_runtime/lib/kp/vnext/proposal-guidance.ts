...
check时，DC、能力/技能、优势劣势、风险及成败意义在decision填写一次；检定决定其结果的那一个observe/social/worldInteraction步骤写进check，success和failure分别是检定成功、失败时这一步的完整结果，两者同样完整具体。steps里有结果的步骤写一个result，并用outcomeBinding=always/onSuccess/onFailure说明它在哪种结果下发生。directSuccess时check各键为[]，steps的outcomeBinding都是always。瞒着在场者的举动或话用concealedCheck代替check，不填DC，由Rules按主要对象的被动察觉定DC：primaryObserverRef填主要瞒着的在场者（通常是交谈对象），检定步骤的success只写动作本身的结果，不写此人是否察觉，failure写此人察觉后的可见反应；observers按冻结上下文里的依据给在场者定注意档位并引用：watching看着行动者或有戒心，unfocused未特别注意，distracted分心（交谈、忙别的、背对、光线暗、离得远），unseen看不到（遮挡、不在视线里），无依据的可不列，按unfocused；叙述中已离开而状态仍在场的定unseen并引用离开的依据。noticedEvidence从察觉者的角度写其会看到或听到的行动者举动，以名字或外貌称呼行动者，不含意图。observe/worldInteraction的结果用entries完整列出实际结果，没有则[]；recordKind只用各自schema提供的类型，推断只属于observe。terminal时check和steps的每个键都是[]。
...
export const VNEXT_PROPOSAL_GUIDANCE_POLICY = deepFreeze({
  version: "zhuwei.proposal-guidance/v57", selection: "flat-type-selection-with-exact-terminal-and-step-surface/v4",
