...
const planRuling = `只提交一个decision。decision.kind=directSuccess时，在steps填写必要操作，observe/social/worldInteraction只填result，不填失败分支或outcomeBinding。observe/worldInteraction的每份结果只用一个entries列表完整列出实际结果，不再填写effects、sensoryEvidence、characterInferences、pressures或opportunities等并列数组；服务器按recordKind分组，没有任何这类结果时明确entries=[]，不得漏掉entries。每种Form只使用其schema列出的recordKind：推断属于observe，不能混入worldInteraction。decision.kind=check时，DC、能力/技能、优势劣势、风险及成败意义在decision一次填写；steps恰好一项observe/social/worldInteraction填写success与failure且outcomeBinding=always，其余后果只填result并用always/onSuccess/onFailure绑定同一次检定。实际成本由所引用Ability或明确的库存、时间操作表达一次，不能只写在summary或重复扣Ability成本。无机械作用的一次手动操作使用worldInteraction及感官证据；操作已有材料不意味着创建Item或Ability。需保留组件关系时必须提交支持的关系操作，不能用描述冒充；机械效果才选相应机械表单。独立观察或思考用observe。仅使用当前工具提供的decision.kind，不填写空adjudication/terminal/proposals。拒绝与澄清保留所选basisRefs；行动的根依据由各step的basisRefs汇总。重大歧义用clarification：只有不同解释会改变重大危险、显著成本、攻击对象或不可逆结果时才提问；风险清楚且意图明确时直接裁决。每个choice填写可公开的label/publicRisk及完整非递归continuation，continuation直接填directSuccess/check及steps，包含已加载类型的完整裁决与后果，inWorldRefusal保留真实尝试成本，cancel无效果。所有分支现在一并冻结和预检，玩家回答仅选择choiceId，不能回答后再生成计划、重选DC/成本/后果。全部分支合计最多16项提案，2–6个选项；不同选项不能相互引用prospective句柄。`;
