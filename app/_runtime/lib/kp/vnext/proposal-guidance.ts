...
首次产生因果证据或机械影响之前，在故事约束及开放授权内固化必要的新对象、事实、Ability、危害或物品。尚无因果或机械作用的环境细节可用commitNarrativeDetail保存为叙述承诺。玩家引用或利用既有承诺时，先用materializeObject或materializeItem固化，再以真实对象或同束prospective引用行动；缺少schema时补取。上下文中与本次操作绑定的叙述承诺必须先物化，不可省略绕过；只回顾已持有知识的knowledgeReview不操作对象。固化须引用原承诺作为依据，保留其名称、描述、位置和原受众；已有materializedRef就继续使用。跨场景和恢复后仍不得改写已承诺的存在、外观、位置、关系或凭空追加危险。与锚点或正史冲突时走可审计更正，不悄悄重写。同名不等于同一对象；不要重复创造。可以没有新发现、没有奖励。模板和故事锚点不是已注册实例或存在证明。
只填写当前工具中所选kind分支实际列出的字段，嵌套对象也按各自分支填写。上下文用于读取和判断，不能因上下文或其他分支出现某个字段就复制到当前结果。引用从对应字段的冻结候选选择；要求选择依据时使用实际支持记录，开放授权或局部不存在的条目本身不是世界事实，使用其中列出的有效支持记录。新对象的本束局部名称在后续类型化引用中精确复用。NPC知识、感官证据及推断的来源按各自表单明确选择，操作目标不自动成为内容证据。
...
const planRuling = `提交三张平表：decision（一个裁决或terminal）、steps（要做的操作，一行一步，不含结果）、results（每条结果一行，写明kind、step序号、branch）。decision.kind=directSuccess时每个observe/social/worldInteraction步骤恰好一行branch=result，步骤的outcomeBinding填always。decision.kind=check时DC、能力/技能、优势劣势、风险及成败意义在decision一次填写；恰好一个observe/social/worldInteraction步骤有success与failure两行且outcomeBinding=always，其余步骤各一行result并用always/onSuccess/onFailure绑定同一次检定。observe/worldInteraction的结果用一个entries列表完整列出实际结果，没有时明确entries=[]，不得漏掉entries；每种Form只使用其schema列出的recordKind：推断属于observe，不能混入worldInteraction。social的结果把回应摊平为responseKind/responseText/responseMotive/responseBasis。terminal决定（passTime、knowledgeReview、clarification、inWorldRefusal、abilityOperation）时steps与results都填[]；clarification的choices.continuation同样使用kind及独立steps/results平表，步骤内不夹带结果。实际成本由所引用Ability或明确的库存、时间操作表达一次，不能只写在summary或重复扣Ability成本。decision.duration冻结本次行动本身消耗的虚构时长，只在none/5min/10min/30min/1h/halfDay里选：角色在做事（交谈、观察、操作、取放）必须选一个档位；只含materialize/commitNarrativeDetail的纯创作束必须填none；遭遇（战斗）进行中也填none——行动者记录里出现encounter字段即表示遭遇进行中；那里的时间由轮次推进，行动只花回合经济，不再另花档位。时长是这次行动本身，不含之后的等待——等待另提passTime。档位：一句问答、一眼、取放物品、一段简短交谈=5min；仔细检查一处、搜查一个房间、一段较长交谈=10min；细致搜查、一场谈判=30min；长途走动、大范围搜索=1h；跨越大半天的活动=halfDay；拿不准取短。服务器按档位在结果之前推进本人所在时间线并公开给场景观察者。无机械作用的一次手动操作使用worldInteraction及感官证据；操作已有材料不意味着创建Item或Ability。需保留组件关系时必须提交支持的关系操作，不能用描述冒充；机械效果才选相应机械表单。独立观察或思考用observe。仅使用当前工具提供的decision.kind，不添加当前分支未列出的字段。拒绝与澄清保留所选basisRefs；行动的根依据由各step的basisRefs汇总。重大歧义用clarification：只有不同解释会改变重大危险、显著成本、攻击对象或不可逆结果时才提问；风险清楚且意图明确时直接裁决。每个choice填写可公开的label/publicRisk及完整非递归continuation，continuation直接填directSuccess/check及steps，包含已加载类型的完整裁决与后果，inWorldRefusal保留真实尝试成本，cancel无效果。所有分支现在一并冻结和预检，玩家回答仅选择choiceId，不能回答后再生成计划、重选DC/成本/后果。全部分支合计最多16项提案，2–6个选项；不同选项不能相互引用prospective句柄。`;
...
const filling: Readonly<Record<VNextProposalCapabilityId, string>> = deepFreeze({
  ...
  formActorPlan: `选择已有NPC，依据其本人冻结self/identity、Knowledge或relationship/promise/debt记录形成timer计划，不能读取其他NPC或玩家秘密；新goal/nextStep是创作，不要求旧记录已有同一句计划。只填明确的目标、下一步、依据、实际依赖资源、正微秒durationMicros、未来执行才会留下的traceDescription以及明确alternateTargetRef/alternateReason。factionRef仅为已有授权势力身份，无势力用{kind:"none"}。只提交表单列出的计划选择。形成不推进时间、不花资源、不立即行动或公开痕迹；不会自动执行替代目标。当前只支持已存在依据与timer，同束新social/worldFact依据和未来未知trigger没有接入，不能猜ID占位。`,
...
export const VNEXT_PROPOSAL_GUIDANCE_POLICY = deepFreeze({
  version: "zhuwei.proposal-guidance/v8", selection: "flat-type-selection-with-exact-terminal-and-step-surface/v4",
