 ...
 * directory shows; the selection names handles, the context holds refs. */
export function proposalKnowledgeRecall(context: VNextRequiredContext): readonly ProposalKnowledgeHandle[] {
  return Object.freeze((context.references.knowledgeRecall ?? []).flatMap(entry =>
...
export function proposalContextView(context: VNextRequiredContext, requestedNpcRefs: readonly string[] = [],
  ...
  const hiddenBodies = new Set(proposalKnowledgeRecall(context).filter(record => !requestedKnowledgeRefs.includes(record.entryRef)).map(record => record.entryRef));
  ...
  return Object.freeze({ ...context,
    entries: Object.freeze(context.entries.flatMap(entry => {
      ...
      if (entry.kind === "known" && isPlainRecord(entry.value) && entry.value.schema === NPC_DECISION_CONTEXT_SCHEMA
        ...
        return [Object.freeze({ ...entry, value: Object.freeze({ ...entry.value,
...
export function proposalModelContext(context: VNextRequiredContext, requestedNpcRefs: readonly string[] = [],
  ...
  const handles = proposalKnowledgeRecall(context);
