...
${RESULT_BOUNDARY_GUIDANCE}
${CHECK_PRESENTATION_GUIDANCE}
...
expression是表达约束。isActorViewer为true才可把行动者称为“你”；其他人物保留正确身份，同名人物按characterRef区分。actorIntent只说明玩家意图，不证明已发生的事实。NPC的声口和公开态度约束措辞，不授权新决定、台词内容或承诺。来源主张必须归因，允许真实、错误、夸张、过时或故意欺骗；忠实表达NPC已经说出的内容，即使其与世界真相不同，也不擅自纠正其事实主张、宣布撒谎或泄露私有动机；句法与用词按中文表达规则修正。不能当作客观真相；不得替玩家决定思想、情绪、台词或下一步。
...
${KNOWLEDGE_EXPRESSION_GUIDANCE}
按该表达要求核对：自然措辞已保留推断及其证据限制时，不因未复述confidence等级而报RESULT_OMITTED或KNOWLEDGE_UPGRADE；正文直接播报内部置信等级或反复作推断免责声明时，报PRESENTATION并摘取具体原文。不要因正常的一处“像是”或必要的限制说明拒绝。越权暗示秘密即使出现在否定句中，也按attribution审核。
${PRECISION_GUIDANCE}
${SOCIAL_RECORD_GUIDANCE}
${RESULT_BOUNDARY_GUIDANCE}
${CHECK_PRESENTATION_GUIDANCE}
...
attribution：NPC、文献、传闻和推测必须保留说话者、来源及把握程度。“某人说Q”不等于Q真实；Q可以错误、夸张、过时或故意欺骗，与世界真相冲突不构成拒绝理由。不得补出未知真假、隐藏动机、他人秘密或改写原话的事实含义；修正语病或换用日常措辞不属于改写事实。SECRET_DISCLOSURE表示越过Viewer或受众权限，KNOWLEDGE_UPGRADE表示partial/full、亲见/转述或不确定性被加强；两者归attribution。知识回顾不是新观察；传递者不是原说话者；full层级不是确定真相。
agency：不能替玩家选择思想、情绪、台词或下一步；只有isActorViewer=true才能将行动者称作你，其他角色按可信身份区分。普通动作实现不等于新增独立决定。
presentation：正文自然清楚、指代可辨、符合已知声口和场景，能回应当前行动。短回应合法；不因个人风格偏好拒绝，也不要求补写压力或悬念。此项只判断可指出的语言问题，不借“更喜欢另一种说法”拒绝正常同义表达。同一说话者、同一付款条件下，“不用你们给钱”和“不用你们付钱”不是不同义务；仅换主语、主动被动或叙述视角，不能据此指控替玩家作决定。是否付款、是否免费、是否只免预付款等实际条件若真被改变，应在results指出原字段、原条件和改变后的条件；不能把语义问题包装成PRESENTATION，也不能仅凭“语气像是”虚构变化。
