...
function sha256Hex(bytes: Uint8Array): string {
  ...
  for (let offset = 0; offset < 8; offset += 1) {
    message[paddedLength - 1 - offset] = Number(bitLength & 0xffn);
    bitLength >>= 8n;
  }
...
/** Synchronous and Worker-safe: replay and projection do not depend on ambient I/O. */
export function canonicalSha256(value: unknown): Sha256Ref {
  return `sha256:${sha256Hex(canonicalProfileBytes(value))}`;
}
