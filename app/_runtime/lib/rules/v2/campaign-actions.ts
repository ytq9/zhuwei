...
function simpleCampaignEvent(
  ...
  switch (input.kind) {
    ...
    case "commitMeaningfulFailure": {
      if (!hasExactKeys(input, ["characterId", "consequences", "factualCause", "goalId", "kind", "methodFingerprint", "proposalId"]) || !isRecord(input.consequences) || ![input.characterId, input.goalId, input.methodFingerprint, input.factualCause].every(isNonEmptyString)) return rejected("invalidRulesInput", "Meaningful failure is not canonical.");
