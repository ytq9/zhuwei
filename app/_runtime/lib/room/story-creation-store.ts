...
export class StoryCreationStore {
  ...
  completeInvocation(input: CompleteStoryInvocation): CompleteStoryInvocationResult {
    return this.atomic<CompleteStoryInvocationResult>(() => {
      const row = this.ownedInvocation(input), completionHash = this.transportHash(input.result);
      ...
      if (supplement && result.kind === "completed" && this.transportHash(parse(row.response_json!)) !== this.transportHash(result.response)) invalid();
      ...
      result.kind, eligible ? 1 : 0, completedAt, result.kind === "completed" ? JSON.stringify(result.response) : null,
  ...
  restoreArchiveSnapshot(input: { source: StoryStoreArchiveSource; snapshot: StoryStoreArchiveSnapshot;
    ...
    return this.atomic<StoryStoreRestoreResult>(() => {
      ...
      for (const row of snapshot.invocations) {
        ...
        call.startedAt, row.leaseUntil, call.completedAt, call.response === undefined ? null : JSON.stringify(call.response),
  ...
  private captureArchive(source: StoryStoreArchiveSource): StoryStoreArchiveSnapshot {
    ...
    return { ...unsigned, snapshotHash: this.transportHash(unsigned) };
  }
  ...
  private validateArchive(snapshot: StoryStoreArchiveSnapshot, source: StoryStoreArchiveSource): void {
    ...
    if (this.transportHash(body) !== snapshotHash) invalid();
  ...
  /** Transport evidence can contain a rejected proposal, including non-NFC
   * text. Preserve it exactly; only semantic inputs use the strict hash port.
   * These bytes also match archiveSha256, whose async API cannot run inside
   * this SQLite transaction. No response text is normalized or repaired. */
  private transportHash(value: unknown): StoryHash {
